...
- **Revisão e Aprovação da Arte (F37, v1.5) - EM EXECUÇÃO em fatias 37.1/37.2/37.3**: fatia 37.1 (Approval Gate + Candidata Única) em execução — valida o modelo de aprovação da arte sem tocar no pipeline de imagem; source of truth `openspec/changes/fase-37-1-approval-gate-candidata-unica/` (decisão do usuário 2026-09-01); 37.2 (correção visual) e 37.3 (correção factual de briefing) em planejamento futuro

...
| 37 | ◆ Revisão e Aprovação da Arte | 15/15 (37.1) | Em execução (37.1 ✅) | 2026-09-01 |
| 37.1 | ✅ Approval Gate + Candidata Única | 15/15 | ✅ Complete | 2026-09-01 |
...
| 50 | ◆ Demonstração Gratuita e Validade dos Créditos (v1.5) | 16/17 | ◆ Executando | Beta fechado; fechamento técnico pendente de restore/UAT/OpenSpec; PJ/jurídico/corte em follow-up pós-PJ |
...
**Goal:** Transformar a primeira entrega da campanha em um **ciclo de aprovação** guiado (experimento beta controlado por feature flag): após a geração, o lojista revisa a arte candidata e a aprova explicitamente antes da entrega (download + Kit de Publicação). Executada em **fatias 37.1/37.2** (padrão F38/38.1/38.2). A **37.1 — Approval Gate + Candidata Única** valida o modelo de aprovação **sem tocar no pipeline de imagem** (mitiga regressão no core); a **37.2 realinhada — Correção Única por Não Conformidade** entrega o segundo caminho (relato de defeito objetivo → no máximo uma v2 sem novo crédito).
...
**Goal:** Criar o **contrato de domínio estruturado** `CampaignBrief` (produto × oferta × mídia × contexto criativo × metadados), separando dados estáveis de produto de dados promocionais da campanha; renomear o wrapper atual de transporte para `ResolvedCampaignContext` (elimina a colisão de nomeação com `brand_profile.campaign_brief`); converter o payload flat na fronteira da rota via mapper único e fazer o pipeline (prompts/copy/review/snapshot) consumir o domínio **preservando o comportamento de geração atual**; snapshot `input_snapshot` **versionado** (`campaign_brief_v1`) por domínio, **nunca contém base64**; `media.images[]` com `role`/`source` (1 imagem `primary` nesta fase); `commercial.validity`/`commercial.legalNotice` estruturados (substituem heurística de string e `mandatoryArtworkText` solto); `creativeContext.themeId?` reservado. Sem reescrever a rota, sem migration SQL, sem catálogo de produtos (fase subsequente).

**Requirements:** derivados dos 10 specs OpenSpec (campaign-brief-contract, campaign-brief-mapper, campaign-brief-snapshot, campaign-brief-pipeline-adapters, transactional-pipeline, mandatory-artwork-text, image-quality-review, ai-image-generation, campaign-types, identity-aware-campaign-briefing)

**Success criteria:**

1. `src/lib/campaign/brief.ts` + `brief-schema.ts` — tipos de domínio (product/commercial/media/creativeContext/metadata) + zod schemas por domínio, invariante de exatamente 1 imagem `primary`; `brief.ts` **sem server-only** (contrato compartilhado)
2. Rename `CampaignBrief` → `ResolvedCampaignContext` (`src/components/campaign/types.ts`) com shape `{ campaignInput, store, brandProfile, identity }` preservado; `buildCampaignBrief` retorna `ResolvedCampaignContext` sem quebrar callers; `brand_profile.campaign_brief` inalterado
3. `buildCampaignBriefFromFlat(input, storeId, source)` — mapper puro na fronteira da rota; campos adormecidos (`hook`/`cta`/`objective`/`targetChannel`/`format`→commercial, `sensitiveConstraints`→creativeContext) com lar canônico 1:1; `validity` string→`commercial.validity`; `mandatoryArtworkText`→`commercial.legalNotice`; `productImageDataUrl`→`media.images[0]` runtime (`role: "primary"`, `source: "upload"`); regra de ausência: campo não informado → ausente (nunca `enabled: false` fabricado)
4. `buildCampaignBriefSnapshot(brief)` — deriva imagens do próprio brief, remove `dataUrl`, tipo `CampaignBriefSnapshotImage` (sem `dataUrl` por construção), `schemaVersion: "campaign_brief_v1"` no ROOT, `metadata` do snapshot **sem** `schemaVersion`; `InputSnapshot` → `CampaignBriefSnapshot`
5. Pipeline consome o domínio nas 4 costuras (prompts/copy/review) + ponte `media.primary.dataUrl` → provider/input-validation (base64 só em memória); conjunto de variáveis de prompt **idêntico**; `buildCommercialRepertoire` decide por `validity.enabled/displayText` (sem heurística); `legalNotice` NÃO entra no copy; revisor recebe legalNotice/validity quando habilitados
6. Orquestração da rota (crédito, rate limit, clearance, readiness, stream, telemetria, estorno) inalterada; `productImageDataUrl` ausente → 400
7. Testes de contrato (brief/mapper/snapshot — 26+ no alinhamento) + regressão completa com co-migração de fixtures (`route.test.ts`, `image-generation-service.test.ts`, `image-review-service.test.ts`, `scripts/benchmark.ts`) + golden tests por intent (offer/spotlight/exclusive)
8. `npx vitest run`, `npm run typecheck`, `npm run lint`, `npm run build` — zero erros

**Dependencies:** F24 (pipeline de créditos), F31.1–F31.3 (intents, prompts, revisor), F38.2.1 (snapshot econômico — padrão de snapshot imutável). Antecede a F37 (Revisão e Aprovação da Arte — consome o snapshot). **Sem** catálogo de produtos (fase subsequente), **sem** UI multi-imagem/câmera, **sem** migration SQL.
...
- **Wave 3** *(blocked on Wave 2)* — 39-04 (mapper `buildCampaignBriefFromFlat` + builder `buildCampaignBriefSnapshot` + testes 8.7-8.14)
- **Wave 4** *(blocked on Wave 3)* — 39-05 (costuras copy + review do domínio)
- **Wave 5** *(blocked on Wave 4)* — 39-06 (costuras image-generation do domínio + golden tests por intent)
- **Wave 6** *(blocked on Wave 5)* — 39-07 (rota: mapper na fronteira + snapshot via builder + co-migração fixtures/benchmark)
...
**Goal:** Reorganizar a montagem do prompt do diretor de imagem dos 4 `.md` por intent em **briefing contextual por blocos** (helper puro `art-director-briefing`), eliminando seções vazias, linhas de tabela em branco, placeholders não resolvidos e duplicações/misturas de naturezas no texto final — separando em seções próprias o texto obrigatório do lojista, o aviso ilustrativo, a identidade visual da loja (preservação), o produto/referências (fidelidade + hierarquia primary × auxiliares), o contexto comercial, as restrições e a direção criativa. **Sem mudança de superfície externa** (UI/form, contrato HTTP/schema/snapshot/domínio e Copy Director inalterados; Revisor ajustado somente no adendo 45-08 para paridade crítica com o Diretor) e **sem migration SQL**. O contrato interno de montagem muda intencionalmente (paridade de keys substituída por invariantes: placeholders ⊆ chaves, determinismo, presença/ausência por bloco, contrato externo intacto + UAT humano comparativo). **Numeração:** F45 = Briefing Contextual do Diretor de Arte (v1.5); **F44 = Temas de Campanha permanece fora da numeração** (adicionada pelo runbook da própria F44).

**Requirements:** derivados dos 2 specs OpenSpec (art-director-contextual-briefing — capability nova — e ai-image-generation — delta MODIFIED/REMOVED)

**Success criteria:**

1. Novo helper puro `src/lib/image-generation/services/art-director-briefing.ts` com os blocos contextuais do diretor (extração sem mudança de comportamento no 45-02; montagem contextual por presença de dados)
2. Os 4 prompts (`campaign-image-director.md` base + `-offer`/`-spotlight`/`-exclusive`) reescritos em camada editorial legível + slots de bloco nomeados; **mão leve** (reorganizar/rotular/remover repetição — preservar o DNA/texto atual)
3. Prompt final contextual: apenas blocos relevantes; nenhuma seção vazia/heading órfão/linha de tabela em branco/placeholder não resolvido; montagem determinística
4. Cada natureza opcional/sensível (validade, texto obrigatório, aviso ilustrativo, detalhes, disponibilidade, restrições) em **um único bloco canônico** — repartição do `buildCommercialRepertoire` (validade → fatos; details/availability → contexto comercial)
5. Texto obrigatório do lojista → `requiredArtworkTextSection` saneada (`{{`→`{`, `}}`→`}`) quando presente; aviso ilustrativo → `illustrativeNoticeSection` (constante única, instrução lateral/mínimo/legível/discreto); ausentes → nada renderizado
6. Identidade (logo/VS) → instrução de preservação (não editar/alterar/redesenhar/distorcer/inventar); text_only → não criar logo/assinatura; `identityImageUrl` permanece provider-only
7. Produto/referências → primary como referência factual forte + auxiliares sem competir + `preserveImageContext` (não-offer); regras anti-invenção e autorização de criatividade preservadas
8. `validatePrompts`/`assemblePrompt` adaptados (zero placeholders residuais por intent; rodapé CORRECT/REGENERATE mantido); specs reconciliadas (REMOVED dos requisitos de paridade; `EXPECTED_KEYS` 38/39)
9. Co-migração: golden tests (39 keys) → invariantes; `validatePrompts`; `prompt-reframe.test.ts` novas âncoras; novos testes `art-director-briefing.test.ts` por bloco presente/ausente; suites irmãs (revisor/copy/form/rota) **sem co-migração** (regressão de não-mudança)
10. `npx vitest run`, `npm run typecheck`, `npm run lint`, `npm run build` — zero erros; UAT humano comparativo (campanhas reais antes/depois: identidade, aviso, texto obrigatório, validade, multi-imagem, oferta)
...
6. Telemetria obrigatória e correta: `AiCallEnvelope` (`extends AiCallInfo`; legado intacto) com `capability`/`protocol`/`status`/`errorType?`; modelo real em validation/review (corrige furo 1); fallback `images.edit` registrado sem usage (`not_available` + duração + custo por unidade); componente da tool somado em `campaign_image` **e** `visual_signature_image` (furo 4); views/RPC de apuração inalteradas
...
- [x] 46-02-PLAN.md — api-keys + gateway (`invoke`/alvo explícito/uma tentativa) + adapters por protocolo + `AiCallEnvelope` (`extends AiCallInfo`) + contrato de erro + sink + mapa capability→generationType (Wave 2)

**Wave 3** *(blocked on Wave 2 completion)*

- [x] 46-03-PLAN.md — Migração das capacidades de TEXTO (copy, correção, brand_profile_text, campaign_spec legado via provider+service+rota) + proprietário único + remoção da persistência manual + co-migração (Wave 3)
...
- [x] 46-05-PLAN.md — Migração das capacidades de IMAGEM (campaign_image via responses; campaign_image_edit como segunda invoke explícita = dois envelopes; visual_signature_image) + furos 3/4 + cost-estimator (tool em campaign_image e visual_signature_image) + fim da soma híbrida manual de imagem (Wave 5) — 272 files / 2702 testes, 4 gates verdes
...
- [x] 46-07-PLAN.md — Remoção das env-vars de modelo/provider do runtime e `.env.example` + extensão do gate (env-var) + co-migração de testes + ordem de deploy (Wave 7) — 275 files / 2720 testes, 4 gates verdes
...
- [x] 46-09-PLAN.md — Verificação final (VERIFICATION.md + UAT.md + registros/arquivamento) (Wave 9)
...
- [x] 48-1-03-PLAN.md — Cenários controlados: schema Zod, fixtures (3 cenários de oferta) e serviço de bootstrap/hash (Wave 2)
- [x] 48-1-04-PLAN.md — Domínio de experimentos prompt-only: schemas, criação com 2 variantes, transições e congelamento (Wave 2)
- [x] 48-1-05-PLAN.md — Harness de gateway: `buildDirectorPrompt`, `LabModelResolver`, `LabPromptLoader`, `LabTelemetrySink` e runtime single-shot (Wave 3)
- [x] 48-1-06-PLAN.md — Persistência de artefatos: bucket/paths, metadados/checksum, URL assinada e cleanup manual (Wave 3)
- [x] 48-1-07-PLAN.md — Execução e snapshots imutáveis: validação técnica (`sharp`), `run-service`, idempotência/reexecução e reserva atômica (Wave 4)
- [x] 48-1-08-PLAN.md — API administrativa sob `/api/admin/laboratorio` (schemas, rotas, estimativa, execução NDJSON e avaliação) (Wave 5)
- [x] 48-1-09-PLAN.md — UI do laboratório: layout/sub-nav, página inicial, criação, detalhe, cenários e execução (Wave 6)
...
- [x] 48-1-14-PLAN.md — UAT local com IA real, migration remota deliberada e verificação final (Wave 9)
...
**Dependencies:** F36 (painel `/loja` em abas + `computeTabUnlock`/`reason-text`), F39/F40/F41/F43 (form de campanha, `validity`/`mandatoryArtworkText`, multi-imagem, revisão do brief), F45 (fonte dos efeitos reais dos campos), F47/F48.1 (precedentes de UI-SPEC, gates e UAT). **Fonte da verdade:** `openspec/changes/archive/2026-09-18-fase-49-ativacao-orientacao-contextual-campos/` (proposal / design D1–D15 / 7 specs / tasks).
**Plans:** 15 plans

**Status:** ✅ **15/15 — Complete (2026-09-18)** — 4 gates verdes (345 arquivos / 3660 testes + 1 skipped; typecheck/lint/build exit 0), UAT humana de compreensão aprovada (9/9 cenários PASS em desktop/375px/320px, sem scroll horizontal e sem poluição visual), decisões editoriais confirmadas (label "Informações obrigatórias na arte" mantido; 8 descrições de tom de voz aprovadas), fences de não-mudança cumpridos (59/59 hashes protegidos, 0 divergências; fence do Diretor de Arte mantido) e **gap closure 49-15** dos achados do code review (ME-01/ME-02/ME-03/LO-03) com 4 gates reexecutados e 59/59 hashes sem divergência.

Plans:
**Wave 1**

- [x] 49-01-PLAN.md — Baseline, inventário de consumidores e fences de não-mudança (Wave 1)
- [x] 49-02-PLAN.md — Conteúdo de orientação em módulos puros + testes unitários (Wave 1)
- [x] 49-03-PLAN.md — Primitivos de ajuda de campo e acessibilidade (Wave 1)

**Wave 2** *(blocked on Wave 1 completion)*

- [x] 49-04-PLAN.md — Loja: dados fiscais, nome público, tom de voz e posicionamento (Wave 2)
- [x] 49-05-PLAN.md — Campanha: descrição do produto, preços, ajuda expansível e feedback dinâmico (Wave 2)
- [x] 49-06-PLAN.md — Informações obrigatórias na arte + revisão do brief separável (Wave 2)
...
- [x] 49-09-PLAN.md — Fences do Diretor de Arte e categorias separáveis da revisão (Wave 3)
- [x] 49-10-PLAN.md — Co-migração de asserções da campanha e da revisão (Wave 3)
- [x] 49-11-PLAN.md — Co-migração de asserções da loja (Wave 3)

**Wave 4** *(blocked on Wave 3 completion)*

- [x] 49-12-PLAN.md — Regressão, 4 gates e prova final de não-mudança (Wave 4)

**Wave 5** *(blocked on Wave 4 completion)*

- [x] 49-13-PLAN.md — UAT humana de compreensão (desktop/mobile) e registro (Wave 5)
...
**Wave 7** *(gap closure do code review; bloqueado pela Wave 6)*

- [x] 49-15-PLAN.md — Gap closure do code review: ME-01 (RECOMMENDED_LABEL), ME-02 (cast-free do tom de voz), ME-03 (ids via useId), LO-03 (trim na revisão) + gates/hashes e tracking 15/15 (Wave 7)
...
Phase 39 (Brief Estruturado de Campanha — v1.5)
                                        ...
                                        Phase 43 (Revisão do Brief Pré-Geração — v1.5) ✅ concluída (15/15, UAT 9/9 PASS)
                                         ...
                                         Phase 48.1 (Laboratório Mínimo de IA — v1.5) ✅ concluída (14/14, 9 waves)
                                                  │
                                                  ▼
                                         Phase 49 (Ativação e Orientação Contextual de Campos — v1.5) ✅ concluída (15/15 — 4 gates verdes, UAT aprovada 9/9, gap closure 49-15)
                                                  │
                                                  ▼
                                         Phase 37 (Revisão e Aprovação da Arte — v1.5)
                                                  │
...
| CONTEXTUAL-FIELD-HELP | Phase 49 | Done ✓ |
| STORE-FIELD-ORIENTATION | Phase 49 | Done ✓ |
| CAMPAIGN-FIELD-ORIENTATION | Phase 49 | Done ✓ |
| STORE-IDENTITY-UI | Phase 49 | Done ✓ |
| CAMPAIGN-INPUT-UI | Phase 49 | Done ✓ |
| MANDATORY-ARTWORK-TEXT | Phase 49 | Done ✓ |
| CAMPAIGN-BRIEF-REVIEW | Phase 49 | Done ✓ |
...
*Last updated: 2026-09-18 — **F49 (Ativação e Orientação Contextual de Campos, v1.5) CONCLUÍDA** (15/15 plans / 7 waves; 4 gates verdes — 345 arquivos / 3660 testes + 1 skipped, typecheck/lint/build exit 0; UAT humana de compreensão aprovada 9/9 em desktop/375px/320px; fences de não-mudança cumpridos — 59/59 hashes, 0 divergências; gap closures 49-14 e 49-15 revalidados; requirements F49 marcados `Done ✓`); antes: **F48.1 (Laboratório Mínimo de IA, v1.5) CONCLUÍDA** (14/14 plans / 9 waves; 48-1-14 concluído — UAT local com IA real **Passos 1–10 PASS** (aprovador Wagner), migration remota aplicada e verificada no projeto `gvbzwihwgzujwsviufgy` (8 tabelas `lab_*` com RLS, 0 grants a `anon`/`authenticated`, bucket `lab-artifacts` privado, RPCs `SECURITY DEFINER` com `search_path=''`, 8 triggers de imutabilidade, `db diff --linked` sem divergência em `lab_*`, `VENDEO_LAB_ENABLED` ausente em produção), 4 gates verdes (333 arquivos / 3588 testes + 1 skipped; typecheck/lint/build exit 0) e contract guard com 0 violações; orçamento 4 runs / 0.458826 USD; fix `tool_choice` descoberto na UAT (`7d6c2f03`); `48-1-VERIFICATION.md` = passed; arquivamento OpenSpec preparado e não executado; deploy não executado — fora do escopo); antes, ainda nesta fase: 48-1-13 concluído — regressão e co-migração (suíte completa verde: 333 arquivos / 3585 testes; guard de contrato congelado `scripts/verify/48-1-13-contract-guard.mjs` com relatório JSON 0 violações; co-migração de testes irmãos não necessária — seam puramente aditivo; typecheck/lint/build verdes); 48-1-12 concluído — suíte de contrato nº 2 (harness de gateway, execução/snapshot imutável, artefatos, API administrativa, UI e snapshots como fixtures determinísticas; 6 arquivos, 142 testes novos); 48-1-11 concluído — suíte de contrato nº 1 (matriz exaustiva dos 5 motivos da guarda, isolamento da produção com detector `forbidden_production_access`, contratos de cenários e do domínio de experimentos e segurança financeira; 5 arquivos de teste, 505 testes em `src/lib/lab`); 48-1-10 concluído — comparação lado a lado e avaliação humana (arte por URL assinada em lote, evidência técnica objetiva por painel, modo cego auditável com `blind_order`, verdict humano append-only com histórico preservado e nenhuma nota automática de qualidade); 48-1-09 concluído — UI do laboratório sob `/admin/laboratorio` (link único na nav do admin, layout com guarda de ambiente + sub-navegação, página inicial com experimentos recentes e pendências, cenários somente leitura, formulário prompt-only com dimensão/modelo fixos e limites travados, detalhe com variantes/runs/budget e painel de execução com estimativa, confirmação explícita e progresso NDJSON; `src/components/ui/` intocado); 48-1-08 concluído — API administrativa `/api/admin/laboratorio` (7 rotas com `apiHandler` + `requireAdmin` + `assertLabEnvironment`, schemas Zod, estimativa por componente com cobertura, leitura com reconciliação preguiçosa e URLs assinadas, execução NDJSON com `confirmed: true` (422), 12 códigos mapeados, idempotência sem nova chamada paga e avaliação humana append-only validada); 48-1-07 concluído — validação técnica objetiva com `sharp`, snapshot imutável antes da reserva, reserva atômica `lab_reserve_run` com os 11 códigos de erro, idempotência por `operationId`, exatamente 1 chamada `campaign_image`/run com terminal em `finally` e reconciliação de órfãos em `pending`/`running`; 48-1-01, 48-1-02, 48-1-03, 48-1-04, 48-1-06 e 48-1-05 concluídos — migration local das 8 tabelas `lab_*` + bucket `lab-artifacts` + triggers de imutabilidade + RPCs `lab_reserve_run`/`lab_create_experiment` (ainda não aplicada no remoto), guarda de ambiente fail-closed + constantes de limite + env vars + gate de arquitetura cobrindo `src/lib/lab/**`, schema `LabScenarioContent` + 3 fixtures controladas de oferta + hash canônico SHA-256 + mapper para `CampaignBrief`/`ResolvedCampaignContext` + bootstrap local idempotente, domínio de experimentos prompt-only (snapshots `official`/`override`, allowlist read-only do catálogo F47, criação atômica de 2 variantes via RPC `lab_create_experiment`, máquina de estados e congelamento), persistência de artefatos (bucket privado `lab-artifacts`, checksum SHA-256 com rollback sem órfão, URL assinada de 3600s e cleanup manual opt-in) e harness de gateway isolado (seam `buildDirectorPrompt`, `LabModelResolver` com alvo fixo, `LabPromptLoader` com override em memória, `LabTelemetrySink` com custo em leitura, `deriveCostCoverage` e runtime single-shot com 1 envelope por run); CONTEXT, UI-SPEC e planos `48-1-01..48-1-14` de `openspec/changes/fase-48-1-laboratorio-ia-minimo/`). F47 concluída (8/8, 287 files / 2782 testes, UAT local aprovado, migration remota verificada, deploy Ready) e F46 concluída (9/9, UAT 8/8). F45 concluída (8/8 plans, 2427 testes, UAT PASS 7/7) e incorporada à F37. **F37.2 realinhada (Correção Única por Não Conformidade) CONCLUÍDA — 19/19 plans (8 waves), 264 files / 2578 testes, 4 gates verdes (vitest/typecheck/lint/build), UAT PASS 9/9 (37.2-6 validado por código), migrations `20260906000001/2/3` aplicadas no remoto; fix pós-UAT `01a7021b` (análise textual + erros 409 legíveis)**; 37.1 concluída (15/15 plans, 2379 testes, UAT PASS 6/6); **37.3 eliminada** (consolidada na 37.2). F44 e Stripe permanecem fora da numeração.*

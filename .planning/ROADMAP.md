...
Phase 39 (Brief Estruturado de Campanha — v1.5)
                                        ...
                                        Phase 43 (Revisão do Brief Pré-Geração — v1.5) ✅ concluída (15/15, UAT 9/9 PASS)
                                         ...
                                         Phase 47 (Catálogo e Seleção de Modelos Admin — v1.5, Change B) ✅ concluída (8/8, 287 files / 2782 testes, UAT local aprovado, migration remota verificada, deploy Ready)
                                                  │
                                                  ▼
                                         Phase 48.1 (Laboratório Mínimo de IA — v1.5) ◆ em execução (3/14, 9 waves)
                                                  │
                                                  ▼
                                         Phase 37 (Revisão e Aprovação da Arte — v1.5)
                                                  │
...
*Last updated: 2026-09-16 — **F48.1 (Laboratório Mínimo de IA, v1.5) em execução** (3/14 plans / 9 waves; 48-1-01, 48-1-02 e 48-1-03 concluídos — migration local das 8 tabelas `lab_*` + bucket `lab-artifacts` + triggers de imutabilidade + RPCs `lab_reserve_run`/`lab_create_experiment` (ainda não aplicada no remoto), guarda de ambiente fail-closed + constantes de limite + env vars + gate de arquitetura cobrindo `src/lib/lab/**` e schema `LabScenarioContent` + 3 fixtures controladas de oferta + hash canônico SHA-256 + mapper para `CampaignBrief`/`ResolvedCampaignContext` + bootstrap local idempotente; CONTEXT, UI-SPEC e planos `48-1-01..48-1-14` de `openspec/changes/fase-48-1-laboratorio-ia-minimo/`). F47 concluída (8/8, 287 files / 2782 testes, UAT local aprovado, migration remota verificada, deploy Ready) e F46 concluída (9/9, UAT 8/8). F45 concluída (8/8 plans, 2427 testes, UAT PASS 7/7) e incorporada à F37. **F37.2 realinhada (Correção Única por Não Conformidade) CONCLUÍDA — 19/19 plans (8 waves), 264 files / 2578 testes, 4 gates verdes (vitest/typecheck/lint/build), UAT PASS 9/9 (37.2-6 validado por código), migrations `20260906000001/2/3` aplicadas no remoto; fix pós-UAT `01a7021b` (análise textual + erros 409 legíveis)**; 37.1 concluída (15/15 plans, 2379 testes, UAT PASS 6/6); **37.3 eliminada** (consolidada na 37.2). F44 e Stripe permanecem fora da numeração.*

...
**Last updated:** 2026-08-14 — **F40 (Campos Comerciais e Avisos do Brief, v1.5) CONCLUÍDA — 9/9 plans, 1997 testes, 4 gates verdes, UAT humano aprovado 6/6**: 40-01 ✅ trackings/renumeração D1 (verificação zero resíduos F40 = Brief Comercial, Stripe → F41); 40-02 ✅ constante única ILLUSTRATIVE_NOTICE_TEXT + checkbox IllustrativeNoticeField + placeholder normalizado; 40-03 ✅ reframe do aviso nos 4 prompts do diretor (hardcode → bloco condicional D6); 40-04 ✅ form state 6 campos novos + helpers exportados + body com validade gated por offer + migração de draft legado; 40-05 ✅ ValidityField presentacional (6 modos) + seções Produto/Oferta/Avisos (D8); 40-06 ✅ testes 1-8 validade + 9-15 aviso + 8.8 brief; 40-07 ✅ testes 16-21 prompt reframe + fixtures co-migradas; 40-08 ✅ route fixtures + regressão completa (1997/1997); 40-09 ✅ verificação final (4 gates + 40-VERIFICATION.md + 40-UAT.md, **UAT aprovado 6/6 — 1 ajuste de UX: placeholder/helper do texto obrigatório alterado a pedido do usuário para evitar duplicação do aviso**). Sem migration SQL (D9 — snapshot campaign_brief_v1 tolerante via jsonb). Fonte da verdade: openspec/changes/fase-40-campos-comerciais-avisos-brief/. **Antecessora concluída: F39 (Brief Estruturado de Campanha) — 8/8 plans, 1950 testes, 4 gates verdes, UAT aprovado 5/5.**
...
**F40 CONCLUÍDA — 9/9 plans, 1997 testes (221 arquivos), 4 gates verdes, UAT humano aprovado 6/6 (1 ajuste de UX: placeholder/helper do texto obrigatório).** Sem migration SQL (D9 — snapshot `campaign_brief_v1` tolerante via jsonb). Fonte da verdade: `openspec/changes/fase-40-campos-comerciais-avisos-brief/`.

### Phase 39 — Brief Estruturado de Campanha ✅ Complete (8 plans / 7 waves)

| Plan | Wave | Status | Description |
|------|------|--------|-------------|
| 39-01 | 1 | ✅ | Trackings / renumeração D1 (F39 = Brief, Stripe → F40) nos 6 arquivos de runbook |
| 39-02 | 1 | ✅ | Contrato de domínio `CampaignBrief` (brief.ts + brief-schema.ts sem server-only) + testes 8.1-8.6/8.15 |
| 39-03 | 2 | ✅ | Rename wrapper `CampaignBrief` → `ResolvedCampaignContext` + `InputSnapshot` → `CampaignBriefSnapshot` (8.21) |
| 39-04 | 3 | ✅ | Mapper `buildCampaignBriefFromFlat` + builder `buildCampaignBriefSnapshot` (8.7-8.14) |
| 39-05 | 4 | ✅ | Costuras copy + review: `mapBriefToCopyDirectorInput` e `ImageReviewInput` do domínio (8.19/8.20) |
| 39-06 | 5 | ✅ | Costuras image-generation: `buildPromptVariables`/`buildCommercialRepertoire` domínio + golden tests por intent (8.16-8.18) |
| 39-07 | 6 | ✅ | Rota: converter flat→domínio na fronteira + `input_snapshot` via builder + co-migração fixtures/benchmark |
| 39-08 | 7 | ✅ | Verificação: vitest/typecheck/lint/build + VERIFICATION.md + UAT humana |

**Source of truth:** `openspec/changes/fase-39-brief-estruturado-campanha/`
**Antercessora:** F38.2.1 (Snapshot Econômico) — precedente de snapshot imutável
**Sucessora:** F37 (Revisão e Aprovação da Arte — consome o snapshot versionado)
...
**Current focus:** Phase 39 — brief-estruturado-campanha
...
| **F38.2.1** | **○ Complete** | **Snapshot Econômico — 7/7 plans: congelar `usd_brl_rate_at_generation`/`credit_value_brl_at_generation` em generation_events, impedir recálculo retroativo, backfill aproximado via audit, verificação I1-I7 53/53 asserts, UAT aprovado** |
...
| F41 | ○ In progress | Mídia de Campanha Mobile (v1.5) — form multi-imagem (primary + auxiliares + câmera), transporte `productImages[]`, persistência dos inputs no storage, provider N input_image, prompt 1+N, validação primary-only, revisor com primary |
...
| 260812-och | Migração de domínio vendeo.tech (PLAN-ONLY) — runbook aprovado: vendeo.tech canônico com beta fechado preservado; Supabase Auth seguro (redirects de vendeo.tech adicionados primeiro preservando beta, Site URL trocado só na janela de corte junto com env+redeploy da main); NEXT_PUBLIC_SITE_URL build-time; /auth/confirm explícito com wildcards complementares; PWA sem mudança de código mas validado por origem; V1/manutenção não removidos cedo; checklists operacional/rollback/UAT, COLLECT FIRST C-01..C-10, decisões D-1..D-7, ordem Supabase→Vercel→DNS→Env/Deploy→Validação→Monitoramento | 2026-08-12 | 0b87ca0 | [260812-och-planejar-migracao-de-dominio-vendeo-tech](./quick/260812-och-planejar-migracao-de-dominio-vendeo-tech/) |
| 260812-qbu | Entry de changelog 2026-08-12 (novo endereço vendeo.tech + página inicial pública com acesso free + beta fechado + app instalável como atalho + card de créditos claro) — content/changelog/2026-08-12-vendeo-em-novo-endereco-acesso-simples.md, copy em português para o lojista sem termos técnicos; ajuste mínimo das expectativas hardcoded do teste de changelog (entries[0] + getLatestAnnouncement); validações verdes (testes 8/8, typecheck, lint) | 2026-08-12 | b4a3268 | [260812-qbu-criar-entry-de-changelog-em-content-chan](./quick/260812-qbu-criar-entry-de-changelog-em-content-chan/) |
| 260814-na1 | Links Novidades na landing pública — link chamativo (accent-green, min-h 44px) abaixo do card "Solicite seu acesso free" + link discreto no rodapé, abrindo MODAL acessível sobre a landing (role=dialog, Esc/backdrop/× fecham, foco ida/volta) com as ~5 entradas mais recentes do changelog resolvidas no server (Home async → getAllEntries().slice(0,5) → props, sem rota nova); máscara progressiva `(11) 99999-9999` no campo WhatsApp via função pura maskWhatsApp (maxLength 15, API/schema intocados); critério editorial permanente: entries de content/changelog agora têm superfície pública; 1967 testes + typecheck/lint/build exit 0 | 2026-08-14 | 980c80c | [260814-na1-adicionar-link-de-novidades-changelog-na](./quick/260814-na1-adicionar-link-de-novidades-changelog-na/) |

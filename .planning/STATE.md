...
**Last updated:** 2026-09-18 — **F49 (Ativação e Orientação Contextual de Campos, v1.5) CONCLUÍDA** (15/15 plans / 7 waves; 4 gates verdes — 345 arquivos / 3660 testes + 1 skipped, typecheck/lint/build exit 0; UAT humana de compreensão **aprovada** — 9/9 cenários PASS em desktop/375px/320px, sem scroll horizontal e sem poluição visual; decisões editoriais confirmadas — label "Informações obrigatórias na arte" mantido e as 8 descrições de tom de voz aprovadas; fences de não-mudança cumpridos — 59/59 hashes, 0 divergências; gap closures 49-14 e 49-15 revalidados — 49-15 endereçou ME-01/ME-02/ME-03/LO-03 do code review); antes: **F48.1 (Laboratório Mínimo de IA, v1.5) CONCLUÍDA** (14/14 plans / 9 waves; **48-1-14 concluído** — UAT local com IA real **Passos 1–10 PASS** (aprovador Wagner, incluindo reteste sem custo do registro de `blind_order`), migration remota aplicada e verificada no projeto `gvbzwihwgzujwsviufgy` (8 tabelas `lab_*` com RLS, 0 grants a `anon`/`authenticated`, bucket `lab-artifacts` privado, RPCs `SECURITY DEFINER` com `search_path=''`, 8 triggers de imutabilidade, `db diff --linked` sem divergência em `lab_*`, `VENDEO_LAB_ENABLED` ausente em produção), 4 gates verdes (**333 arquivos / 3588 testes + 1 skipped**; typecheck/lint/build exit 0) e contract guard com 0 violações; orçamento 4 runs / `0.458826` USD; fix `tool_choice` descoberto na UAT (`7d6c2f03`, debug arquivado); `48-1-VERIFICATION.md` = **passed**; arquivamento OpenSpec preparado e não executado; deploy não executado — fora do escopo); antes, ainda nesta fase: **48-1-13 concluído** — regressão e co-migração: suíte completa verde (333 arquivos / 3585 testes), guard de contrato congelado (`scripts/verify/48-1-13-contract-guard.mjs`, relatório JSON com 0 violações), co-migração de testes irmãos **não necessária** (seam puramente aditivo) e os 3 gates typecheck/lint/build verdes; **48-1-12 concluído** — suíte de contrato nº 2 (harness de gateway, execução/snapshot imutável, artefatos, API administrativa, UI e snapshots como fixtures determinísticas; 6 arquivos, 142 testes novos, 4 gates verdes); **48-1-11 concluído** — suíte de contrato nº 1 (matriz exaustiva dos 5 motivos da guarda, isolamento da produção com detector `forbidden_production_access`, contratos de cenários e do domínio de experimentos e segurança financeira; 5 arquivos de teste, 505 testes em `src/lib/lab`); **48-1-10 concluído** — comparação lado a lado e avaliação humana: evidência objetiva por painel (arte por URL assinada em lote, status técnico, custo coerente com a cobertura, latência, bytes, dimensões, alertas técnicos e erro sanitizado), modo cego auditável com `blind_order`, verdict humano append-only (`baseline|candidate|tie|none` + observação) com histórico preservado e nenhum julgamento automático de qualidade; 7 arquivos, 15 testes jsdom novos, 4 gates verdes); **48-1-09 concluído** — UI do laboratório sob `/admin/laboratorio`: um único link na navegação principal do admin + sub-navegação interna própria (Experimentos/Cenários/Avaliações), guarda de ambiente avaliada antes de qualquer leitura nas 4 páginas (estado desabilitado com `reason` legível e nenhum acesso a `lab_*`/storage/provider), página inicial com experimentos recentes e avaliações pendentes, lista de cenários somente leitura, formulário prompt-only com dimensão `prompt` e modelo fixo não editáveis e limites travados (3 cenários, repetições 1–3, teto 1–12), detalhe com variantes/runs/budget e painel de execução com estimativa exibida, confirmação explícita obrigatória, `confirmed: true` + `operationId` idempotente e progresso NDJSON até o `runId`; 8 primitivos locais criados e `src/components/ui/` intocado (10 arquivos) — 17 arquivos, 24 testes jsdom novos, 4 gates verdes); 48-1-01 concluído — migration local das tabelas/bucket/triggers/RPCs do laboratório, ainda não aplicada no remoto; 48-1-02 concluído — guarda de ambiente fail-closed com 5 motivos, constantes de limite travadas, env vars documentadas em `.env.example` e gate de arquitetura cobrindo `src/lib/lab/**`; 48-1-03 concluído — schema `LabScenarioContent` com rejeição `unsupported_scenario_mode`, 3 fixtures controladas de oferta com imagens geradas localmente, hash canônico SHA-256 e bootstrap local idempotente (`created: 3` → `created: 0`, hashes conferidos contra o banco local); 48-1-04 concluído — domínio de experimentos prompt-only (schemas Zod com rejeição `unsupported_changed_dimension`, snapshots de prompt `official`/`override` com SHA-256, allowlist read-only do catálogo F47, criação atômica de 2 variantes via RPC `lab_create_experiment`, máquina de estados travada e congelamento após o primeiro run; 69 testes no domínio); 48-1-06 concluído — persistência de artefatos (bucket privado `lab-artifacts`, path próprio validado, metadados + checksum SHA-256 com rollback sem órfão, URL assinada de 3600s e cleanup manual opt-in que protege runs em andamento); 48-1-05 concluído — harness de gateway isolado (seam aditivo público `buildDirectorPrompt` reutilizando `buildPromptVariables`+`assemblePrompt` sem tocar o fluxo de produção, `LabModelResolver` com alvo fixo e precedência em `campaign_image` + delegação read-only fora do escopo, `LabPromptLoader extends PromptLoader` servindo o snapshot da variante em memória sem escrever em `prompts/`, `LabTelemetrySink` em `src/lib/ai/**` com `resolveAiCost` em modo leitura preservando a `CostResolution` real completa e sem `generation_events`/`AiCostTracker.record`, `deriveCostCoverage` (`complete|partial|missing`) e runtime single-shot com exatamente 1 envelope por run — 50 testes novos, 45 files / 715 testes verdes nos gates de `src/lib/lab`+`src/lib/ai`+`src/lib/image-generation`); **48-1-07 concluído** (execução real e snapshots imutáveis: validação técnica objetiva com `sharp` sem nenhuma nota de qualidade — `structuredOutputValid`/`ocrAlert` sempre `null` e falhas viram alertas; snapshot imutável montado **antes** da reserva e nunca vazio; reserva atômica `lab_reserve_run` com os 11 códigos de erro mapeados e `run_sequence` sempre derivado no banco; idempotência por `operationId` sem reexecução e reexecução explícita por `supersedes_run_id`; **exatamente 1 chamada `campaign_image` por run** com estado terminal garantido em `finally`; falha de artefato → `artifact_persistence_failed` sem órfão; reconciliação preguiçosa de runs órfãos em `pending` **e** `running` após `LAB_RUN_STALE_MS`; 313 testes verdes em `src/lib/lab` — 6 arquivos novos); CONTEXT, UI-SPEC e planos `48-1-01..48-1-14` gerados a partir de `openspec/changes/fase-48-1-laboratorio-ia-minimo/`); antes: **F47 (Catálogo e Seleção de Modelos Admin, v1.5, Change B) CONCLUÍDA** (8/8 plans, 287 files / 2782 testes, 4 gates verdes, UAT local aprovado, migration remota verificada, deploy de produção Ready; fonte `openspec/changes/fase-47-catalogo-e-selecao-de-modelos-admin/`); F45 concluída e incorporada à branch da F37; F37.1 concluída; **F37.2 realinhada (Correção Única por Não Conformidade) CONCLUÍDA** (19/19 plans, 8 waves, 264 files / 2578 testes, 4 gates verdes, UAT 9/9 PASS, migrations `20260906000001/2/3` no remoto, fix pós-UAT `01a7021b`; fonte `openspec/changes/fase-37-2-correcao-unica-por-nao-conformidade/`); **37.3 eliminada** (consolidada na 37.2); **48-1-08 concluído** (API administrativa do laboratório sob `/api/admin/laboratorio`: schemas Zod anexados a `src/lib/admin/schemas.ts` sem tocar os existentes, helper `estimateLabCampaignImageCost` em `src/lib/ai/**` (único path permitido pelo gate) + `estimateExperimentPlan` por componente com cobertura `complete|partial|missing` e sem bloquear por pricing incompleto, camada de leitura com reconciliação preguiçosa de runs órfãos no detalhe do experimento e URLs assinadas (3600s) no detalhe do run, serviço de avaliação humana append-only validando mesmo experimento/cenário, estado terminal e papéis `baseline`/`candidate`, wiring `prepareExperimentRun`/`runPreparedExperimentRun` (reserva antes de qualquer chamada paga) e **7 rotas** com `apiHandler` + `requireAdmin` + `assertLabEnvironment` — 403 `environment_blocked` antes de qualquer leitura, 422 `confirmation_required` antes do parse completo, 12 códigos mapeados (400/403/404/409/500), operação idempotente 200 sem stream e NDJSON com `runId` no evento final; 95 testes verdes no gate da API, `tsc -p tsconfig.typecheck.json --noEmit` e `lint` exit 0).
**Current phase:** 50
**Last activity:** 2026-09-23 - Phase 50: task 17.3 concluída; gates técnicos do beta fechado registrados, com seis follow-ups pós-PJ ainda abertos
...
### Phase 50 — Demonstração Gratuita e Validade dos Créditos ◆ Executando (17/17 plans / 10 waves; escopo F50 concluído; escopo pós-PJ transferido; pronta para verificação final e arquivamento)

**Planejada em 2026-09-20** a partir da base OpenSpec `openspec/changes/fase-50-demonstracao-gratuita-e-validade-dos-creditos/` (proposal / design D1–D26 / 27 specs / tasks 50-01..50-17 / legal-package). A F50 entrega a base técnica da demonstração em beta fechado, sem novos usuários: `VENDEO_DEMO_CREDITS_ENABLED=false`, email ausente/false, `VENDEO_PUBLIC_SIGNUP_ENABLED=false`, documentos v1.5/v1.4/v1.2 não publicados e sem cobrança/checkout/monetização. 17 planos em 10 waves; o restore real de 17.2 e os gates técnicos de 17.3 foram validados, e o escopo F50 está pronto para verificação final/arquivamento. PJ, placeholders/datas, sincronização final, jurídico, autoack aprovado, publicação, ativação, admissão e smoke pós-corte foram transferidos para quick pós-PJ ainda não criada. **Fences:** sem alterar prompts, gateway de IA, snapshot, domínio ou contrato de geração (402); bônus sempre não-expirável na F50.

**Execução:** 50-01 ✅ (trackings + baseline de não-mudança + inventário de consumidores de saldo/`balance` — `50-BASELINE.txt` com SHA_INICIAL_F50=14ccb8bc…, 79 hashes SHA-256, 13 consumidores de `balance` classificados migrar/não-afetado; nenhum arquivo de produção alterado; commit `65894705`). 50-02 ✅ (migration estrutural `20260920000001_f50_demo_credits.sql` — 5 colunas demo + CHECK composto, tipos `demo`/`expiration` (9), `benefit_type='demo'`, 4 tabelas novas (`credit_notifications`/`product_events`/`support_credit_requests`/`data_subject_requests`) com RLS/grants/índices únicos + `privacy_notice_version`; aplicada localmente e verificada; commit `30d52c81`; sem docs legais). 50-03 ✅ (RPCs SQL — `try_grant_demo_entitlement`/`grant_demo_credits`/`materialize_demo_expiration` + reescrita `grant_credits`/`reserve_credit`/`refund_credit` + 3 wrappers demo + `admin_exception` bônus + neutralização da criação admin sem CNPJ + **12 RPCs restritas a service_role**; `db reset --local` reaplicado e **smoke tests comportamentais PASS** — `available` 4 ramos, grant único/retry `already_granted`, idempotência `type`+`amount` (`idempotency_conflict`), expiração idempotente, `anon`/`authenticated` negados (incl. `admin_create_store_for_user`); commits `1824795e`/`e6ce369a`/`18d17bac`/`4017bfcd`/`fbf261c2`). 50-04 ✅ (serviços de saldo/demo, elegibilidade incluindo `admin_exception`, flags fail-closed, `demo-status`, migração do admin summary e 46 testes focados; commits `bc878bea`/`2e2ff66c`/`1b5acee4`). 50-08 ✅ (ProductEventService best-effort/fail-open, dedup, erros Supabase registrados e 4 testes; commits `4e873b0e`/`087f1a22`). 50-10 ✅ (documentos consolidados Terms v1.5/Privacy v1.4/AUP v1.2, **template de publicação fora de supabase/migrations**, mecanismo reaceite/ciência verificado; fornecedor/PJ pendente para o corte; commit `356e5673`). 50-15 ✅ (storage hardening, smokes HTTP de acesso público/assinatura/renovação/limpeza, 43 testes focados verdes, falha de assinatura pré-mutação nos fluxos de IA; commits `51351230`/`7c27c06b`/`7c6dc61d`). 50-17 ✅ (runbook de backup externo e continuidade em `docs/operations/backup-runbook.md`; teste real de restore e gates permanecem no corte 50-14).

**Execução adicional:** 50-05 ✅ (rotas demo e `first_generation` implementados, payload D14 corrigido, mocks co-migrados e 100/100 testes relacionados).

**Artefatos:** `50-CONTEXT.md` (síntese fiel D1–D26), `50-UI-SPEC.md` (consolidado do OpenSpec), `50-01-PLAN.md` .. `50-17-PLAN.md`, `50-BASELINE.txt`, `50-01-SUMMARY.md`, `50-02-SUMMARY.md`, `50-03-SUMMARY.md`, `50-04-SUMMARY.md`, `50-05-SUMMARY.md`, `50-08-SUMMARY.md`, `50-10-SUMMARY.md`, `50-15-SUMMARY.md`, `50-17-SUMMARY.md`.
**Fonte da verdade:** `openspec/changes/fase-50-demonstracao-gratuita-e-validade-dos-creditos/`.
**Próximo passo:** checkpoint humano após 50-16; nenhum plano posterior antes de aprovação explícita.

### Phase 49 — Ativação e Orientação Contextual de Campos ✅ Complete (15/15 plans / 7 waves)

**Planejada em 2026-09-18** a partir da base OpenSpec `openspec/changes/archive/2026-09-18-fase-49-ativacao-orientacao-contextual-campos/` (proposal / design D1–D15 / 7 specs / tasks). Fase de **apresentação/conteúdo apenas** — orientação contextual no próprio campo (hint inline, descrição contextual da opção, ajuda expansível, feedback dinâmico) nos formulários de loja (`/loja`) e campanha (`/campanhas/nova`, incluindo a revisão F43). 15 planos em 7 waves (49-14 = gap closure da microcopy/placeholder de "Informações obrigatórias na arte"; 49-15 = gap closure dos achados do code review ME-01/ME-02/ME-03/LO-03); plan-check **PASSED** (0 blockers, 0 warnings); cobertura 100% dos grupos 1–9 do `tasks.md` e 7/7 capabilities. **Sem** migration, prompts, gateway/modelos, schemas, snapshot, domínio, contrato HTTP ou banco; sem novas chamadas de IA ou validadores semânticos.

**Artefatos:** `49-CONTEXT.md` (síntese fiel D1–D15 + verificação de código), `49-UI-SPEC.md` (consolidado do OpenSpec), `49-PATTERNS.md` (análogos exatos), `49-01-PLAN.md` .. `49-15-PLAN.md`.
...
**Execução:** 49-01 ✅ (baseline de não-mudança — `49-BASELINE.txt` com SHA_INICIAL_F49, inventário de consumidores, regra de desbloqueio e 59 hashes SHA-256; nenhum arquivo de produção alterado); 49-02 ✅ (módulos puros de conteúdo `src/lib/store-onboarding/field-guidance.ts` + `src/lib/campaign/field-guidance.ts` + `priceFeedbackMessage` de 4 estados + 2 arquivos de teste puros, 13 testes verdes; nenhum arquivo de produção existente alterado); 49-03 ✅ (primitivos locais `FieldHint` + `RecommendedBadge` + `ExpandableHelp` acessível colapsado por padrão com `aria-controls` sempre válido, touch ≥ 44px e foco visível + 3 arquivos de teste jsdom (11 testes verdes; suíte `src/__tests__/components/ui/` 36 testes verdes); nenhum arquivo existente de `src/components/ui/` modificado; convenção `aria-required="true"` sem `required` nativo registrada); 49-04 ✅ (aba Dados com subseção `Dados fiscais` + microcopy de nome público; Tom de Voz com hint/complements e descrição contextual da opção selecionada; Posicionamento renomeado com hint/identity e exemplo expansível; Descrição Curta recomendada e Slogan opcional; `aria-required` sem `required` nativo e `aria-describedby` por `useId`; 17 testes das suites de loja verdes; `tabs.ts`/`reason-text.ts` intocados); 49-05 ✅ (campanha: "Descrição do produto" com microcopy/placeholder reais + hint; labels "Preço anterior (original)"/"Preço de venda (final)" com hints; bloco permanente das 3 regras substituído por `ExpandableHelp` colapsado; feedback dinâmico de 4 estados via `priceFeedbackMessage` com id único e `tone` amber no estado "só preço anterior"; obrigatoriedade acessível `aria-required` sem `required` nativo em Nome do Produto/Preço de venda/Selo e grupo da imagem primária com `role="group"` + `aria-labelledby`/`aria-describedby`; 2 arquivos modificados, 51 testes de campanha/revisão verdes, typecheck/lint limpos; `use-campaign-form.ts`/`inferIntent`/`availableOptions`/`ValidityField` intocados); 49-06 ✅ (`MandatoryArtworkField` com label "Informações obrigatórias na arte", microcopy positiva e placeholder multi-linha real de produto via `field-guidance`, `rows=3` e hint associado por `aria-describedby`/`useId` — id estático/`value`/`onChange`/`maxLength=200` preservados, campo visível e sem advertências negativas; revisão do brief com a seção Avisos em itens distintos e rotulados (`ILLUSTRATIVE_NOTICE_TEXT` com ícone `Check` × `MANDATORY_ARTWORK_LABEL` + texto livre com `whitespace-pre-line`) e rótulos de preço "Preço anterior"/"Preço de venda"; chamada/import mortos de `buildMandatoryArtworkText` removidos do componente — helper intacto; 2 arquivos modificados, 32 arquivos/325 testes verdes e typecheck limpo; fences de body/snapshot/contrato HTTP comprovadas por `git diff`). 49-07 ✅ (testes de orientação/a11y/persistência: 3 arquivos jsdom criados — `store-identity-form.orientation.test.tsx` (6), `campaign-input-form.orientation.test.tsx` (11) e `mandatory-artwork-field.test.tsx` (3) — 20 testes verdes; asserções consomem as strings canônicas dos módulos `field-guidance` (fonte única); `aria-describedby` composto (hint/descrição/feedback/erro) com ids resolvendo para elementos existentes, `aria-invalid` nos erros e `aria-required="true"` sem `required` nativo; descrição contextual das 8 opções de tom de voz; ajuda expansível colapsada por padrão (`aria-expanded`/`hidden`); grupo real da imagem primária (`role="group"` + `aria-labelledby`/`aria-describedby`); feedback dos 4 estados de preço (incl. "só anterior" neutro sem "Sem preço"); placeholder multi-linha e transporte preservando `\n`; nenhum arquivo de produção modificado; `tsc -p tsconfig.typecheck.json --noEmit` exit 0); 49-09 ✅ (fences do Diretor de Arte e categorias separáveis da revisão: `product.description` chega ao `CopyDirectorInput` e ausente nas 7 seções do diretor + não-mudança de body/snapshot/transporte; revisão do brief com aviso ilustrativo e informações obrigatórias como itens separáveis nos 4 casos (quebras de linha preservadas); correspondência da microcopy de identidade × consumidores reais — copy direto + guarda de fonte dos 4 módulos de perfil/direção visual; 2 arquivos de teste criados, 17 testes verdes; nenhum arquivo de produção alterado); 49-12 ✅ (regressão de comportamento verde — 13 arquivos / 114 testes, exit 0; 4 gates verdes — `npx vitest run` 345 arquivos / 3660 testes + 1 skipped, typecheck/lint/build exit 0; prova final de não-mudança — 59 hashes SHA-256 sem divergência, `git diff --name-only <SHA_INICIAL_F49>..HEAD` restrito à apresentação/conteúdo, `product.description` ausente do `art-director-briefing`; evidência em `49-GATES.txt`; nenhum arquivo de produção alterado); 49-14 ✅ (gap closure de apresentação/conteúdo — achado da UAT no cenário 7: `MANDATORY_ARTWORK_HINT` passa a citar "restrições"/"preferencialmente uma linha para cada item" e `MANDATORY_ARTWORK_PLACEHOLDER` mantém 3 linhas reais com a 3ª = "Venda proibida para menores"; base OpenSpec (proposal/design/2 specs) e 49-CONTEXT/49-PATTERNS alinhados; gap registrado em `49-UAT.md`; 4 gates verdes e 59/59 hashes sem divergência registrados em `49-GATES.txt`; **único arquivo de produção alterado:** `src/lib/campaign/field-guidance.ts`; fase **permanece pendente da re-UAT humana** do 49-13).
**Concluída em 2026-09-18:** 15/15 plans / 7 waves; 4 gates verdes (345 arquivos / 3660 testes + 1 skipped; typecheck/lint/build exit 0); UAT humana de compreensão **aprovada** (9/9 cenários PASS em desktop/375px/320px, sem scroll horizontal e sem poluição visual) e decisões editoriais confirmadas; fences de não-mudança cumpridos (59/59 hashes protegidos, 0 divergências; fence do Diretor de Arte mantido); gap closures 49-14 e 49-15 revalidados (49-15 endereçou ME-01/ME-02/ME-03/LO-03 com co-migração de asserções de ids). **Próximo passo:** arquivamento OpenSpec do change e planejamento da próxima fatia F48.x (F48.2–F48.6), conforme autorização.

### Phase 48.1 — Laboratório Mínimo de IA ✅ Complete (14/14 plans / 9 waves)

**Concluída em 2026-09-17.** UAT local **aprovado** (Passos 1–10 PASS, aprovador Wagner) e **migration remota aplicada e verificada** (projeto `gvbzwihwgzujwsviufgy`); 4 gates verdes (**333 arquivos / 3588 testes + 1 skipped**; typecheck/lint/build exit 0) e contract guard `scripts/verify/48-1-13-contract-guard.mjs` com **0 violações** (base `75ab54cf…`). Schema remoto: 8 tabelas `lab_*` com RLS, **0** grants a `anon`/`authenticated`, bucket `lab-artifacts` privado, RPCs `SECURITY DEFINER` com `search_path=''`, 8 triggers de imutabilidade e **0 divergências de schema em `lab_*`** (`db diff --linked`); `VENDEO_LAB_ENABLED` ausente em produção (superfície inerte). Orçamento do UAT: 4 runs / `0.458826` USD (inclui a tentativa diagnóstica pré-fix). Desvios: D-01 e D-02 resolvidos; D-03/D-04 residuais não-bloqueantes; D-05 (drift pré-existente, fora do escopo). Verificação: `.planning/phases/48.1-laboratorio-ia-minimo/48-1-VERIFICATION.md` (**passed**); UAT: `48.1-UAT.md`. **Arquivamento OpenSpec preparado e não executado.** Deploy **não** executado (fora do escopo).
...
| 48-1-11 | 7 | ✅ | Testes 1: domínio, guardas, isolamento e cenários (suíte de contrato nº 1 — 5 arquivos, 142 testes novos, 4 gates verdes) |
| 48-1-12 | 7 | ✅ | Testes 2: harness, execução, artefatos, API, UI e avaliação (suíte de contrato nº 2 — 6 arquivos, 142 testes novos, 4 gates verdes) |
| 48-1-13 | 8 | ✅ | Regressão e co-migração de fixtures: suíte completa verde (333 arquivos / 3585 testes + 1 skipped), guard de contrato congelado `scripts/verify/48-1-13-contract-guard.mjs` (base `48.1-BASELINE.txt`, nunca HEAD) com relatório JSON (156 arquivos alterados, 0 violação), typecheck/lint/build verdes e **zero co-migração** (seam 100% aditivo) |
| 48-1-14 | 9 | ✅ | UAT local com IA real (**Passos 1–10 PASS**, aprovado por Wagner), migration remota deliberada aplicada/verificada e verificação final (`48-1-VERIFICATION.md` = passed); fix `tool_choice` descoberto na UAT (`7d6c2f03`) com sessão de debug arquivada |
...
### Phase 47 — Catálogo e Seleção de Modelos Admin (Change B) ✅ Complete (8/8 plans / 7 waves)
...
| Plan | Wave | Status | Description |
|------|------|--------|-------------|
| 37-1-01 | 1 | ✅ | Trackings F37 em fatias 37.1/37.2 (6 arquivos + ROADMAP raiz; source of truth `openspec/changes/fase-37-1-approval-gate-candidata-unica/`) |
| 37-1-02 | 1 | ✅ | Migrations: `campaign_art_versions` + colunas em `campaigns` + índice 1-approved + RPC `approve_campaign_art_version` + db push [BLOCKING] |
| 37-1-03 | 2 | ✅ | Flag `campaign_approval_enabled` (constante + ALL_FEATURE_FLAG_KEYS + isCampaignApprovalEnabled fail-closed) |
| 37-1-04 | 2 | ✅ | Tipos `CampaignArtVersion` + persistência `createArtVersion`/`listArtVersions` |
| 37-1-05 | 3 | ✅ | `ApprovalDisplayState` + `computeApprovalState` + `isDeliveryReleased` + candidata ativa |
| 37-1-06 | 3 | ✅ | generate-image insere v1 (flag on, fail-safe) |
| 37-1-07 | 4 | ✅ | Gates download + publication-copy (403) |
| 37-1-08 | 4 | ✅ | Rota `POST /api/campaign/[id]/approve` |
| 37-1-09 | 5 | ✅ | UI: page.tsx + `CampaignApprovalView` + client.tsx |
| 37-1-10 | 4 | ✅ | Testes estados (13.1-13.7) |
| 37-1-11 | 5 | ✅ | Testes rota approve (14.1-14.4) |
| 37-1-12 | 5 | ✅ | Testes generate-image v1 + download/copy gated (15/16.x) |
| 37-1-13 | 6 | ✅ | Testes UI revisão (17.1-17.5) + co-migração páginas |
| 37-1-14 | 7 | ✅ | Regressão + co-migração fixtures (suíte completa) |
| 37-1-15 | 8 | ✅ | 4 gates + UAT + VERIFICATION.md |
...
**Current focus:** Phase 49 — ativacao-orientacao-contextual-campos (CONCLUÍDA; próxima fatia: arquivamento OpenSpec / F48.x)
...
| Phase 48.1 P48-1-01 | 26min | 3 tasks | 3 files |
| Phase 48.1 P48-1-02 | 4 min | 3 tasks | 6 files |
| Phase 48.1 P48-1-03 | 7 min | 3 tasks | 15 files |
| Phase 48.1 P48-1-04 | 7 min | 3 tasks | 8 files |
| Phase 48.1 P48-1-05 | 12 min | 3 tasks | 13 files |
| Phase 48.1 P48-1-09 | 23 min | 3 tasks | 17 files |
| Phase 48.1 P48-1-10 | 12 min | 3 tasks | 7 files |
...
| Phase 49 P03 | 2 min | 3 tasks | 6 files |
| Phase 49 P04 | 5 min | 3 tasks | 1 files |
| Phase 49 P05 | 5 min | 3 tasks | 2 files |
| Phase 49 P06 | 2 min | 3 tasks | 2 files |
| Phase 49 P10 | 2 min | 1 task | 1 files |
...
Phase: 49 (ativacao-orientacao-contextual-campos) — COMPLETE (15/15)
Plan: 15 of 15
...
| F37 | Concluida | RevisÃ£o e AprovaÃ§Ã£o da Arte (v1.5): fatia 37.1 Approval Gate + Candidata Ãšnica concluÃ­da â€” 15/15 plans, 2379 testes, 4 gates verdes, UAT PASS 6/6; 37.2 realinhada CONCLUIDA; 37.3 eliminada |
...
| F42 | âœ… Completed | Signup Controlado e Elegibilidade Freemium (v1.5) â€” 20/20 plans, 2182 testes, 4 gates verdes, UAT 20.5â€“20.15 PASS: signup pÃºblico controlado (Google OAuth + email/senha), Turnstile, callback PKCE, kill switch duplo, motor elegibilidade revisado (situacao_nao_ativa/dados_oficiais_incompletos/cidade-UF/CNAE), admin reviews rico, legal v1.4/v1.3 |
| F43 | âœ… Complete | RevisÃ£o do Brief PrÃ©-GeraÃ§Ã£o (v1.5) â€” 15/15 plans, 2317 testes, 4 gates verdes, UAT 15.5â€“15.13 PASS (9/9): gate client-side de revisÃ£o do brief, compressÃ£o antes da revisÃ£o, helpers puros, override `brief_review_confirmed`, flag admin `force_brief_vision_check`, resumo Produto/Oferta/Imagens/Avisos/Custo + loja/marca + slot Tema |

| F45 | ✅ Complete | Briefing Contextual do Diretor de Arte (v1.5) — 8/8 plans (45-01..45-08, 5 waves), 2427 testes (253 files), 4 gates verdes, UAT comparativo antes×depois PASS (7/7) + artes reais aprovadas como publicáveis: montagem contextual do prompt do diretor por blocos (`art-director-briefing`), sem seções vazias/duplicações, sem mudança de superfície externa; adendo 45-08 (alinhamento Diretor × Revisor, identidade canônica com área segura + concordância de gênero) |

> **Fora da numeraÃ§Ã£o (iniciativa diferida, v1.7+):** MonetizaÃ§Ã£o pÃºblica / Stripe â€” reaberta quando houver condiÃ§Ã£o real de executar (empresa, jurÃ­dico, contabilidade, operaÃ§Ã£o fiscal, decisÃ£o de monetizaÃ§Ã£o). NÃ£o Ã© fase numerada.
...
| 260820-nl8 | Quick 2 Preço/Helper (formulário de campanha) — label do campo de desconto "Preço com Desconto *" → "Preço Final" SEM asterisco fixo, SEM condicional por campaignIntent e SEM "(opcional)" (alinhado ao modelo "os preços orientam a intenção"); helper sempre visível sob o título "Oferta" em linhas discretas (div + p por linha, text-xs, sem card/tooltip) com microcopy: "Preço original + preço final = Oferta; Somente preço final = Oferta ou Destaque; Sem nenhum preço preenchido = Destaque ou Exclusividade"; painel de ajustes (campaign-adjustments-panel.tsx) com label "Preço Final"; teste de render novo (label, ausência de asterisco, ausência de "(opcional)" escopado ao label do campo, helper, painel). Sem mudança em contrato/backend/schema/rota/inferência. 3 arquivos, 5 testes novos, 242 files/2230 testes verdes, typecheck/lint limpos | 2026-08-20 | 1fa0f8ce | [260820-nl8-quick-2-preco-helper-ajustar-comunicacao](./quick/260820-nl8-quick-2-preco-helper-ajustar-comunicacao/) |
| 260820-pl1 | Reviewer Multi-Imagens Autorizadas — revisor passa a receber TODAS as referências do lojista: `ImageReviewService.review`/`callVisionModel` trocam `primaryImageDataUrl` por `referenceImageDataUrls[]` primary-first (gerada + todas as referências, detail high), linha de instrução dinâmica singular (1 ref) / plural "as imagens de referência autorizadas" (2+ refs) / ausente (sem refs), seção `referenceImagesContextSection` (só count>=2) com regra de hierarquia (referência adicional autoriza apoio/variação mas não substitui o herói principal; produto fora de TODAS as referências e dos dados = invented_information crítico), critério #5 do prompt atualizado, call site propaga `mediaImagesDataUrls(brief)`. Task 1 feat c971ae22 + Task 2 test b701e68e (10 cenários, mocks sem modelo real, testes 23/23b F41 co-migrados); 62 focados + 243 files/2255 testes verdes, typecheck/lint limpos. Fora de escopo: copy e validação de data. | 2026-08-20 | b701e68e | [260820-pl1-reviewer-multi-imagens-autorizadas-ajust](./quick/260820-pl1-reviewer-multi-imagens-autorizadas-ajust/) |
| 260820-p3u | Validação de Data Passada na validade da oferta — bloquear no frontend, antes do fetch/geração, validade anterior a hoje (compare ISO YYYY-MM-DD por string): helper exportado `getTodayISO(now = new Date())` (data local YYYY-MM-DD, único new Date() do módulo); validators de validade com param opcional `todayISO` e checagem de passado que SÓ roda em until-date/range (today/stock/custom retornam null mesmo com datas preenchidas — draft antigo trocado para custom não bloqueia); mensagens inline "Data final/inicial não pode ser anterior à data de hoje"; igual a hoje permitido; regra start<=end e mensagem de ordem originais preservadas. Sem mudança de contrato/backend/schema/rota/prompts/copy/reviewer/UI. 2 arquivos, 27 testes verdes no teste focado + 217 regressão fluxo, typecheck/lint limpos | 2026-08-20 | d6e2e76d | [260820-p3u-valida-o-de-data-passada-na-validade-da](./quick/260820-p3u-valida-o-de-data-passada-na-validade-da/) |
| 260820-pae | Copy Director Precisão Comercial — impedir invenção de condições comerciais, canais de venda e promessas operacionais não informados nos 4 prompts de copy: bloco curto "### Precisão comercial" em tom de direção criativa ("fatos protegidos", categorias de condições proibidas por categoria, "Não transforme inferências em fatos comerciais", CTAs neutros de loja física, criatividade explicitamente permitida) inserido em base/offer/spotlight/exclusive entre "Campos obrigatórios" e "Regras de tom de voz"; exemplos de CTA do offer/base "Garanta já a sua!/Corra e aproveite!/Clique e compre!" → "Garanta já o seu!/Aproveite na loja!/Fale com a equipe!" (spotlight/exclusive mantidos); novo teste copy-director-prompt.test.ts por âncoras conceituais (6 testes, ausência total de "Clique e compre"). Sem mudança em schema/mapper/service/backend/rota/form/reviewer. 2 tasks, 4 prompts + 1 arquivo de teste novo, vitest 6+24+5 verdes + typecheck/lint limpos + grep gate 0 ocorrências de "Clique e compre" | 2026-08-20 | bc9b4cec | [260820-pae-copy-director-anti-invencao-comercial-ad](./quick/260820-pae-copy-director-anti-invencao-comercial-ad/) |
| 260820-q1y | Preview de Imagens Sem Recorte no Upload de Campanha — troca `object-cover`→`object-contain` em célula `aspect-square` + `bg-bg-elevated` no preview do CampaignImageUpload (padrão repo visual-signature-approval-modal), mantendo badges Principal/Câmera, botão Remover, clique para adicionar, contrato de props e MAX_CAMPAIGN_IMAGES; teste render novo (5 cenários: object-contain presente, object-cover ausente, badges via within(preview), onRemove(id), smoke de erro); 3 testes irmãos com mocks `() => null` intactos + typecheck/lint limpos + git diff = exatamente 2 arquivos. UI-only: pipeline/reviewer/director/compressão/HEIC-EXIF/schema/rota/storage/limites/copy/validação de data fora de escopo | 2026-08-20 | 87f40a1c | [260820-q1y-preview-de-imagens-sem-recorte-no-upload](./quick/260820-q1y-preview-de-imagens-sem-recorte-no-upload/) |
| 260820-t0o | Topbar mobile compacta — hamburger nunca espremido/oculto em telas pequenas (`shrink-0` no botão, mantido `min-h/min-w 44px` + `md:hidden`); CTA "Nova Campanha" icon-only no mobile (`min-w-[44px]`, `justify-center`, `gap-0 px-2`, texto `hidden sm:inline` preservado no DOM, `aria-label="Criar nova campanha"`); container direito (CTA + conta) com `shrink-0`; trigger da conta compacto (`min-w-[44px]`, `justify-center`, `gap-0 px-2`, email `hidden sm:inline-block` no DOM, `aria-label="Menu da conta"`); espaçamento responsivo (header `px-3 sm:px-4`, containers `gap-2 sm:gap-3` / `gap-1 sm:gap-3`); sem overflow-x-hidden global, sem mudança em sidebar/drawer/layout. 3 tasks atômicas (a4ecf1d1, e797a355, 62caba86), 5 testes novos, 245 files/2277 testes verdes, typecheck/lint/build exit 0. Pendência: checkpoint manual em 320px/375px | 2026-08-20 | 62caba86 | [260820-t0o-corrigir-topbar-mobile-hamburger-encober](./quick/260820-t0o-corrigir-topbar-mobile-hamburger-encober/) |
| 260820-r4r | Landing OAuth Google — rebrand da homepage pública p/ verificação OAuth: H1 "Vendeo" (era "Campanhas profissionais para lojas físicas") + slogan "Postou, vendeo!" secundário + funcionalidade "O Vendeo é uma plataforma de marketing para pequenos e médios lojistas" c/ IA comercial + seções novas "O Vendeo pode criar" (Arte promocional, Texto e chamada, Legenda, CTA) e "Como funciona" (4 passos: Cadastre sua loja → Informe sua oferta → O Vendeo cria a campanha → Revise e publique) + rodapé Termos de Uso/Política de Privacidade/Contato mailto; branches CTA flag on/off e data-public-signup-enabled preservados byte a byte; 9 testes novos (19 no arquivo), 245 files/2272 testes verdes, typecheck/lint/build limpos. UAT manual aprovado 8/8 (conferência URL Google OAuth incluída) | 2026-08-20 | c93f9aa0 | [260820-r4r-alinhar-homepage-p-blica-do-vendeo-aos-r](./quick/260820-r4r-alinhar-homepage-p-blica-do-vendeo-aos-r/) |
| 260820-qpk | AppShell Viewport, Scroll e Overflow Mobile — scroll do documento no lugar do scroll aninhado do `<main>`: wrapper `h-screen`→`min-h-dvh` (viewport dinâmica mobile), `<main>` remove `overflow-auto` + ganha `min-w-0` (barra do navegador cobre a página inteira), coluna central mantém `min-h-0` + ganha `min-w-0` (decisão do usuário), sidebar desktop `sticky top-0 h-dvh` (fixa com scroll próprio), topbar `sticky top-0 z-30` (visível ao rolar; abaixo do drawer z-40/modais z-50), viewport export explícito `width: device-width` + `initialScale: 1` + themeColor (hardening defensivo — Next já mescla defaults; sem maximumScale), co-migração `lg:sticky lg:top-8`→`lg:top-20` no painel legal do onboarding (regressão evitada em /loja); testes A/B/C no app-shell.test.tsx (3 existentes intactos; B localizado no trecho do main) + novo layout-viewport.test.ts (RED→GREEN demonstrado); 245 files/2264 testes verdes + typecheck/lint/build limpos. Nenhum arquivo fora de escopo (globals.css/sidebar-drawer intactos; docs não relacionados não commitados). UAT manual aprovado pelo usuário (2026-08-20): desktop /dashboard e /campanhas/nova com barra da página inteira + sidebar/topbar sticky; mobile 375/320px sem corte/zoom-out e drawer congela fundo; /loja com painel legal visível sob a topbar; sem scroll horizontal | 2026-08-20 | 537610e6 | [260820-qpk-planeje-o-quick-appshell-viewport-scroll](./quick/260820-qpk-planeje-o-quick-appshell-viewport-scroll/) |
| 260821-qcw | Ajustes de controles operacionais no admin, auditoria e labels humanizados — flag `captcha_enabled` (seed `true` + fallback `envVarBool("VENDEO_CAPTCHA_ENABLED", true)`, fail-safe: migration/falha de leitura nunca desligam captcha por acidente) aplicada a login/signup/recuperação via `FeatureFlagService.readFlag` (`captchaEnabled` removido do LaunchConfig; env var vira fallback/override); controles de geração (campanhas + assinatura visual) movidos de Configurações Econômicas para Controles Operacionais (`getCost`/`getAllCosts` resolvem enabled das flags fail-open F38 D5, PUT operation-costs sem `enabled` — schema `.strict()` + costCredits obrigatório + RPC `p_enabled: null`, coluna "Habilitada" e `saveEnabled` removidos do form); labels humanizados "Geração de campanha"/"Geração de assinatura visual" (key técnica como subtexto mono) em admin/costs; labels de auditoria `feature_flag_update`/`operation_cost_update` + `feature_flag`/`operation_cost` em labels.ts com testes; GET /api/admin/feature-flags retorna lista (`.in(ALL_FEATURE_FLAG_KEYS)`) e tela Controles operacionais renderiza 4 flags com labels humanizados + descriptions do banco (limite Supabase Auth visível). 2333 testes, typecheck/lint/build limpos, migration de seeds aplicada no remoto | 2026-08-21 | 45d91692 | [260821-qcw-implementar-ajustes-de-controles-operaci](./quick/260821-qcw-implementar-ajustes-de-controles-operaci/) |
| 260902-kqo | Separar semanticamente o aviso fixo "Imagem meramente ilustrativa" do texto obrigatório livre do lojista APENAS na montagem/instrução do diretor de imagem — `splitDirectorLegalText` (função pura module-scope, prefixo determinístico da constante canônica `ILLUSTRATIVE_NOTICE_TEXT` de constants.ts:1; 3 casos: aviso-only → `mandatoryArtworkText=''` + `illustrativeNotice` canônico; prefixo canônico+`\n` → separa aviso × texto livre; free-only/legado → texto integral) aplicada no único ponto de montagem (`buildPromptVariables`); `mandatoryArtworkText` (só texto livre) + chave nova `illustrativeNotice` sempre presentes no mapa (golden 38→39 keys, `toHaveLength(39)` ×5, casos novos (a)(b)(c)); 4 prompts do diretor com linha de tabela `| **Aviso ilustrativo** | {{illustrativeNotice}} |` + instrução curta aprovada (texto mínimo, legível e discreto, separado dos demais textos, preferencialmente nas laterais da arte) substituindo a frase mista; frase do campo "Texto obrigatório na arte" mantida byte a byte; literal canônico removido dos 4 `.md` (grep 0 — fonte única via placeholder injetado server-side, check B invertido no prompt-reframe.test.ts). UI, contrato HTTP, schema público, snapshot/domínio (`legalNotice.text` integral concatenado) e revisor de imagem intocados — suítes dessas superfícies verdes SEM edição. 33 golden + 105 regressão + 60 rota, typecheck/eslint limpos | 2026-09-02 | e4cde72a | [260902-kqo-separar-semanticamente-o-aviso-fixo-imag](./quick/260902-kqo-separar-semanticamente-o-aviso-fixo-imag/) |
| 260902-mqj | Corrigir qualidade do fallback de imagem OpenAI - quando o caminho Responses falha e o pipeline cai no fallback `images.edit`, a arte NÃO degrada mais a identidade visual da loja: `fallbackToImageApi` agora monta `files` em ordem determinística `[primary, auxiliares de productImagesDataUrls[1..] com dedupe, identity por último]` e envia ao `openai.images.edit` (1 imagem → escalar/wire atual; 2+ → array). Gate F41 D7 relaxado: `isSinglePrimary` (exigia length==1, premissa "1 base image" superada — SDK openai@^6.39.0 `image: Uploadable | Array<Uploadable>` até 16 imagens p/ GPT image models incl. gpt-image-2) substituída por `canUseEditFallback` (exige apenas primary presente) nos 2 call sites (pre-response attempt>=1 e pós-erro isResponsesApiError); fetch da identidade mantido — falha → bloqueio com erro PT-BR existente (nunca gera arte sem assinatura visual), OK → identity por último; helper module-scope `parseEditImageDataUrl` (regex única png/jpeg/webp, mensagem parametrizada); auxiliar inválida → erro explícito (sem descarte silencioso); TODO removido; log operacional mínimo console.error (model, images=<n>, identityIncluded, promptChars — sem telemetria). Testes 18/18b co-migrados (gate relaxado + dedupe do shape real de produção), 1/19/19b reforçados com shape do body.image, novos 20-23 (multi-referência 4 arquivos na ordem exata, bloqueio identidade, auxiliar malformada, primary inválida). Prompts/UI/schema/HTTP/domínio-snapshot/revisor intocados (diff = 2 arquivos); 12/12 provider + 85/85 src/lib/image-generation + typecheck limpos | 2026-09-02 | 7df02e0e | [260902-mqj-fix-openai-image-fallback-quality-preser](./quick/260902-mqj-fix-openai-image-fallback-quality-preser/) |

## Decisions

- [Phase 49]: Manter o label "Informações obrigatórias na arte" — a alternativa "Detalhes obrigatórios na arte" foi descartada porque o campo abrange características, detalhes **e** restrições (decisão editorial confirmada na UAT humana, aprovador Wagner, 2026-09-18) (49-13)
- [Phase 49]: Aprovar a redação final das 8 descrições de tom de voz exatamente como entregues em `src/lib/store-onboarding/field-guidance.ts` (decisão editorial confirmada na UAT humana, 2026-09-18) (49-13)
- [Phase 48.1]: F48.1/48-1-09: a UI do laboratório é a barreira financeira do lado do cliente — a estimativa é buscada e exibida, o POST com `confirmed: true` + `operationId` (UUID em estado, reutilizado por fingerprint do payload e renovado quando variante/cenário/repetição mudam) só sai após o ConfirmDialog; o ramo idempotente (HTTP 200 JSON sem `x-ndjson`) exibe o `runId` existente sem nova chamada paga (48-1-09)
- [Phase 48.1]: F48.1/48-1-09: a sub-navegação interna do laboratório usa `<nav>` + 3 `<Link>` e o primitivo `tabs` do UI-SPEC foi deliberadamente omitido — nenhuma tela o consome e criar componente sem consumidor seria escopo morto (a omissão está registrada no UI-SPEC) (48-1-09)
- [Phase 48.1]: F48.1/48-1-09: badges de status usam apenas as variantes `ready`/`default`/`error` do primitivo existente — a variante `generating` contém a substring "rating" e violaria a varredura plan-level `score|nota|publicável|rating` (48-1-09)
- [Phase 48.1]: F48.1/48-1-09: imports nomeados aliasados (`getLabEnvironment as readLabEnvironment`, `listScenarioVersions as readScenarioVersions`) para satisfazer literalmente os critérios de aceitação de contagem ("1 ocorrência por arquivo" / "1 chamada") sem renunciar à convenção de import nomeado do repositório (48-1-09)
...
- [Phase 48.1]: F48.1 e LOCAL-FIRST: as 8 tabelas lab_*, o bucket privado lab-artifacts, os 8 triggers de imutabilidade e as RPCs lab_reserve_run/lab_create_experiment existem apenas no Supabase local ate o push remoto deliberado do 48-1-14 (D16).
- [Phase 48.1]: F48.1/48-1-02: guarda de ambiente estritamente fail-closed — a flag exige igualdade estrita com a string 'true' e o bloqueio de hosts de producao (*.supabase.co/.in/.com) e avaliado ANTES da allowlist, que portanto nunca habilita producao (D2). — Qualquer ambiguidade de configuracao deve recusar; allowlist e para hosts de desenvolvimento (ex.: host.docker.internal), nunca para producao.
- [Phase 48.1]: F48.1/48-1-02: a guarda expoe apenas o hostname (supabaseHost) e a mensagem do LabEnvironmentError contem o reason + hostname — nunca URL completa, path, query ou chave; assertLabEnvironment() devolve o estado e lanca em qualquer reason diferente de 'ok' (D2/D15, T-48-1-12). — Erros de configuracao nao podem vazar a URL do projeto nem chaves para logs/telas.
- [Phase 48.1]: F48.1/48-1-02: constantes de limite em src/lib/lab/limits.ts como fonte unica (MAX_SCENARIOS_PER_EXPERIMENT=3, MAX_REPETITIONS=3, MAX_RUNS_PER_EXPERIMENT=12, DEFAULT_MAX_RUNS_PER_EXPERIMENT=6, MAX_CONCURRENT_LAB_RUNS=1, LAB_RUN_STALE_MS=900000, LAB_ARTIFACT_RETENTION_DAYS=30), em modulo puro sem server-only, importavel por UI/servicos/testes (D14). — Evita literais duplicados entre dominio, API, UI e banco; o teto 12 e divisivel por MAX_REPETITIONS=3 (4 combinacoes).
- [Phase 48.1]: F48.1/48-1-02: gate de arquitetura estendido de forma estritamente aditiva para src/lib/lab/** (sem generation_events/AiCostTracker.record, sem provider de imagem de producao/SDK-wire, sem leitura direta de chave) + gate de sanidade; DELIVERY_MARKER_FILES permanece com as 5 entradas originais (D6/D15, T-48-1-14). — O sink do laboratorio (src/lib/ai/lab-telemetry-sink.ts, plano 48-1-05) ja cai na regra de prefixo src/lib/ai/** para resolveAiCost, sem excecao nova.
- [Phase 48.1]: F48.1/48-1-02: o catalogo canonico docs/operations/environment-variables.md nao foi alterado — apenas .env.example recebeu NEXT_PUBLIC_SUPABASE_URL, VENDEO_LAB_ENABLED=false e VENDEO_LAB_ALLOWED_SUPABASE_HOSTS (DV-6), sem secrets reais. — O doc foi lido apenas como referencia de convencao; o plano lista .env.example como unico arquivo de documentacao a modificar.

...
### Phase 41 — Mídia de Campanha Mobile ✅ Complete (13 plans / 6 waves)

| Plan | Wave | Status | Description |
|------|------|--------|-------------|
| 41-01 | 1 | ✅ | Trackings D1 (grep-verificação renumeração F41/F42, zero resíduos, registro commit 195b467) |
| 41-02 | 1 | ✅ | Config + Transporte schema (MAX_CAMPAIGN_IMAGES=4 + teto agregado; ProductImageInputSchema + productImages[] + productImageDataUrl optional) |
...
**F41 CONCLUÍDA — 13/13 plans, 4 gates verdes (222 files / 2033 testes, +36 vs F40), UAT humano 6/6 cenários (Android validado em produção ✅; iOS HEIC pendente de confirmação final).** Sem migration SQL (D5 — snapshot `campaign_brief_v1` tolerante + bucket `campaign-images` existente). Fonte da verdade: `openspec/changes/fase-41-midia-de-campanha-mobile/`.
...
| 260815-i9a | Alinhar concessão mensal freemium por raiz de CNPJ (3 tasks, PLAN-ONLY inicial → executado após revisão r1+r2) — reescrita `grant_monthly_credits` (migration `20260815000001`) com: máx. 1 grant/raiz/ciclo, recipiente determinístico (matriz preferida, senão loja não-teste mais antiga), limiar de elegibilidade (NÃO teto, `bonus_balance < p_bonus_cap` → grant integral; `>=` → nenhum), ciclo por aniversário em `America/Sao_Paulo` com clamp 29/30/31, idade mínima por dia civil BR, ordem checks existência→limiar→INSERT via `try_grant_monthly_entitlement` (ON CONFLICT = anti-corrida), REVOKE/GRANT service_role, shape canônico com `roots_considered`; botão admin `body.skipped === true` + testes shape canônico + script de verificação `39-monthly-grant-por-raiz-verification.mjs` (driver `pg`, `DATABASE_URL`); specs/docs/env alinhados. 18/18 greps Task 1, vitest 13/13, typecheck/lint limpos. Pendências: aplicar migration (Local+Remote) e rodar script de verificação em staging/dev (requer DATABASE_URL) | 2026-08-16 | cefbfe0 | [260815-i9a-objetivo-planejar-sem-executar-o-alinham](./quick/260815-i9a-objetivo-planejar-sem-executar-o-alinham/) |

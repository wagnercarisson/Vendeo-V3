...
**F37 em execução** â€” fatia 37.1 (Approval Gate + Candidata Ãšnica, v1.5) em andamento â€” valida o modelo de aprovaÃ§Ã£o da arte **sem tocar no pipeline de imagem**: flag `campaign_approval_enabled` (fail-closed), tabela `campaign_art_versions` + colunas de aprovaÃ§Ã£o em `campaigns`, RPC `approve_campaign_art_version` transacional, generate-image insere v1 quando a flag ligada, estado `ApprovalDisplayState`/gating (download + copy 403 atÃ© aprovar), tela de revisÃ£o da candidata ativa, rota approve. **Sem correÃ§Ã£o em qualquer forma** (37.2/37.3). Source of truth: `openspec/changes/fase-37-1-approval-gate-candidata-unica/`.

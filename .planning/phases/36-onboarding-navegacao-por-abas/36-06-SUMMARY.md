...
key-decisions:
  ...
  - "Pós-entrega (drift re-drift após 'Manter e salvar'): gate de interceptação/resume passou a ser por DRIFT ATIVO (driftStatus 'new' / criticalDriftStatus 'new'), não por driftCategory — 'Manter e salvar' grava drift_dismissed_snapshot (via ignorar()) e o mesmo drift não reabre na navegação por abas. Fixes A/B/C: use-onboarding-tabs.ts (gate), use-drift-detection.ts (espelho local dismissedSnapshot), store-identity-form.tsx (persistSaveFromDrift persiste no caminho de navegação antes do /realign). Spec store-onboarding-autosave atualizada (L120-141 + cenários Fix A/B/C)."
  - "Pós-entrega (crítico x banco estale antes do save — F36): o drift CRÍTICO passou a ser computado CLIENT-SIDE contra o formData vivo (computeCriticalDriftStatus em drift-validator.ts, paridade exata com o servidor), porque o GET visual-signature avalia contra o BANCO — antes de um PATCH ele via 'none' e o save explícito da aba Dados passava sem interceptar. GET expõe input_snapshot/art_direction.content_used/dismissed_snapshot/credit_balance/credits_charging_enabled; POST /dismiss-critical-drift aceita body { snapshot } (valores aceitos, fallback banco); 'Gerar novamente' persiste dados aceitos ANTES de abrir a aprovação; gate de nova assinatura por CRÉDITOS (canGenerateNewSignature = !creditsChargingEnabled || saldo>0) substituindo o limite legado de 3 (totalGeneratedSignatures vira só contagem). Sem crédito, modal oferece 'Ver meus créditos' (→/conta). Escopo fechado dentro da F36 (sem abrir frente paralela)."

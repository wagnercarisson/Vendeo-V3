# Phase 38.1: Apuração de Custos de IA por Entrega — Context

**Gathered:** 2026-08-08
**Status:** Ready for planning
**Source:** OpenSpec change artifacts (`openspec/changes/fase-38-1-ai-cost-accounting/`)
...
| # | Furo | Evidência |
|---|------|-----------|
| 1 | Copy da campanha grava `estimated_cost_usd: NULL` (usage descartado) | `route.ts:593` chama `estimateAiCost` sem usage; `CopyDirectorService` descarta o usage que o `TextProviderResult` já traz |
| 2 | `metadata.totalCost` grava o **nome do provider** em vez do custo | `route.ts:642` |
| 3 | Modelos reais sem preço na tabela TS: `gemini-3.1-flash-lite` e `gpt-image-2` → `estimateAiCost` retorna `null`/fallback cego | `cost-estimator.ts` |
| 4 | Revisão (`ImageReviewService`) e validação (`InputValidationService`) — vision gpt-4o cara — **somem da contabilidade** | só vão ao `MetricsWriter` |
| 5 | Evento de assinatura visual sem `estimated_cost_usd` nem tokens, apesar de o provider retornar usage | `generation-events.ts` |
| 6 | `attempt_number` sempre 1, mesmo com até 3 tentativas no loop real de revisão/recomposição | `image-generation-service.ts` |
| 7 | `duration_ms` representa o pipeline inteiro, não a chamada individual | `route.ts:555` |
...
- **Semântica econômica por domínio** — campanha: reprovações/regenerações do mesmo request são a **mesma entrega** (run persistido em `campaigns.operation_run_id`, preparando reuso pela F37); assinatura visual: cada geração é uma entrega debitável separada (nova tentativa pós-falha técnica = novo `operation_run_id`)
...
- **Trackings atualizados** (`.planning/STATE.md`, `.planning/ROADMAP.md`, `.planning/REQUIREMENTS.md`, `.planning/PROJECT.md`, `ROADMAP.md` raiz) registrando a F38.1 como desdobramento da F38 — ver runbook na D1

**Dependências:** F24 (ledger `credit_transactions` — reconciliação por leitura), F25/F28 (pipeline + telemetria existentes, `admin_get_metrics`), F29.1.1 (VS), F37 (aprovação — decisão, não custa IA), F38 (`credit_operation_costs` — eixo créditos), F39 (Stripe — consumirá o custo real apurado aqui).

**Relação com a F38 (eixo de créditos):** F38 define "quanto o usuário paga/debita" (`credit_operation_costs`, créditos); a 38.1 responde "quanto essa entrega custou para o Vendeo" (`generation_events`, USD estimado). As duas camadas **não se misturam** e se encontram apenas nas views de reconciliação (D10).
...
- **`pricing_version`:** UUID da linha `ai_model_pricing` usada, `'code_default'` quando veio do fallback em código, `NULL` quando não se aplica
...
- **Regra de registro:** tokens/usage são gravados **sempre** que existirem, mesmo que o custo em USD fique `NULL` (ex.: `not_available`).
...
- **Mudança de comportamento importante:** hoje `estimateAiCost` pode retornar `null`. Na 38.1, o `AiCostTracker` **sempre grava o evento**; o `estimated_cost_usd` é `NULL` apenas quando `cost_source = 'not_available'`.
...
- Tipos **existentes mantidos** (backward compat): `campaign_pipeline`, `campaign_copy`, `campaign_image`, `visual_signature`, `brand_profile_without_logo`, `brand_profile_with_logo`.
...
- **Typographic fallback da VS: NÃO gera evento call-level** (não inventar chamada — D5).

### D6 — Duração por chamada e por entrega
`DECIDIDO`

- **`generation_events.duration_ms` = duração da chamada individual** (novo padrão).
- **Duração da entrega** = `SUM(duration_ms)` por `operation_run_id` via views (D10). Sem tabela `operation_runs` (D1).
- **Correção do furo 7:** hoje os eventos de campanha usam `durationMs` do pipeline inteiro (`route.ts:555`) — passa a ser medido **por chamada** no ponto de execução.
- O delivery marker **não grava custo nem tokens** (anti-dupla-contagem, D1): mantém apenas `duration_ms` da entrega (pipeline) com `metadata.duration_is_pipeline: true` — para não quebrar `admin_get_metrics` que usa `AVG(duration_ms)` dos eventos `campaign_pipeline`. Views de custo **somam apenas call-level**.
...
- **Padrão de exposição de usage:** os serviços expõem `usage`/custo via **callback opcional**:
  ```typescript
...
- **Delivery marker sem custo:** `record` do delivery recebe `cost: null`/sem `estimatedCostUsd` e sem tokens — a duração fica no `durationMs` com `metadata.duration_is_pipeline: true`.
- **Geração nunca é bloqueada por telemetria** — qualquer falha de escrita é logada e ignorada.
...
- **RPC `admin_set_ai_model_price`** (SECURITY DEFINER, `SET search_path=''`, transacional):
  ...
  ```
  - **`p_reason` vem antes dos parâmetros com `DEFAULT`** (regra de assinatura do Postgres).
...
O `estimateAiCost` deixa de ser a única fonte e passa a ser o **resolvedor** de custo `resolveAiCost`:

```
resolveAiCost(provider, model, usage?, providerReportedCostUsd?) → CostResolution {
...
- **Correções incorporadas:** copy com `usage` real (furo 1); `gemini-3.1-flash-lite` e `gpt-image-2` na tabela/seeds (furo 3); cached tokens (`input_tokens_details.cached_tokens`) e image tokens (`output_tokens_details.image_tokens`) contabilizados; `manual_unknown` para preço manual.
...
| `src/lib/image-generation/services/image-generation-service.ts` | `onMetricsEvent`/novo callback expõe `usage` (do `generateWithRetry`) e `attempt_number` por chamada (image + review); tracker registra `campaign_image` e `campaign_image_review` |
...
|---|---------|---------------|
| 1 | `.planning/STATE.md` | Adicionar seção "Phase 38.1 — Apuração de Custos de IA por Entrega" (após a Fase 38) com plans/status; atualizar "Current Position" e a linha mencionando a F38.1 como desdobramento; frontmatter "Last updated" |
| 2 | `.planning/ROADMAP.md` | Adicionar linha na tabela Progress: `38.1` e seção de detalhes "Phase 38.1" (goal/success criteria/dependencies, fonte `openspec/changes/fase-38-1-ai-cost-accounting/`); atualizar contagem de fases da milestone v1.5 e rodapé "Last updated" |
...
### Apuração de Custos de IA por Entrega — Core
- `openspec/changes/fase-38-1-ai-cost-accounting/design.md` — Full architecture decisions D1-D12 + estrutura de arquivos + riscos + migration plan
- `openspec/changes/fase-38-1-ai-cost-accounting/proposal.md` — Why/What/Impact + capabilities novas/modificadas
- `openspec/changes/fase-38-1-ai-cost-accounting/tasks.md` — Complete task breakdown (8 grupos: migration, core library, serviços onCall, rotas, admin APIs, testes, verificação, runbook trackings)

### Specs (13)
- `openspec/changes/fase-38-1-ai-cost-accounting/specs/ai-cost-tracker/spec.md` — tipos centrais + AiCostEvent + AiCallInfo + AiCostTracker (D1/D4/D7)
- `openspec/changes/fase-38-1-ai-cost-accounting/specs/ai-cost-estimator/spec.md` — resolveAiCost por fonte (D9)
- `openspec/changes/fase-38-1-ai-cost-accounting/specs/ai-model-pricing/spec.md` — tabela + seeds + RPC + GET/PUT admin (D8)
- `openspec/changes/fase-38-1-ai-cost-accounting/specs/ai-cost-accounting/spec.md` — views + admin_cost_vs_credits + admin_get_ai_costs + GET /api/admin/ai-costs (D10)
- `openspec/changes/fase-38-1-ai-cost-accounting/specs/transactional-pipeline/spec.md` — delta traceId/operation_run_id + telemetria + attempt/duration + totalCost + reconciliação (D1/D2/D6/D7)
- `openspec/changes/fase-38-1-ai-cost-accounting/specs/store-visual-signature/spec.md` — delta VS + custo/tokens + visual_signature_id + novo run pós-falha (D1/D5/D11)
- `openspec/changes/fase-38-1-ai-cost-accounting/specs/ai-image-generation/spec.md` — delta onMetricsEvent usage/attempt/durationMs (D11)
- `openspec/changes/fase-38-1-ai-cost-accounting/specs/copy-director/spec.md` — delta onCall usage (D11)
- `openspec/changes/fase-38-1-ai-cost-accounting/specs/image-quality-review/spec.md` — delta onCall usage review (D11)
- `openspec/changes/fase-38-1-ai-cost-accounting/specs/validation-review-alignment/spec.md` — delta onCall usage input validation (D11)
- `openspec/changes/fase-38-1-ai-cost-accounting/specs/store-brand-profiler-without-logo/spec.md` — delta onCall brand profiler (D11)
- `openspec/changes/fase-38-1-ai-cost-accounting/specs/text-only-brand-inference/spec.md` — delta onCall text-only (D11)
- `openspec/changes/fase-38-1-ai-cost-accounting/specs/brand-director-prompt/spec.md` — delta onCall brand director (D11)

### Dependências de fases anteriores
- `.planning/phases/24-creditos-schema-saldo-transacoes/24-CONTEXT.md` — F24 reserve_credit, credit_balances, credit_transactions, CreditService
- `.planning/phases/25-integracao-transacional-pipeline/25-CONTEXT.md` — F25 generate-image route, pipeline 3 zonas
- `.planning/phases/28-observabilidade-operacao-launch-controls/28-CONTEXT.md` — F28 launch config, admin_get_metrics, estimateAiCost, generation_events schema original
- `.planning/phases/29-1-1-creditos-assinatura-visual/29-1-1-CONTEXT.md` — F29.1.1 generate-without-logo + credit_tx_id
- `.planning/phases/38-credit-operation-costs/38-CONTEXT.md` — F38 credit_operation_costs, OperationCostService, rotas com custo dinâmico
</canonical_refs>

<specifics>
## Specific Ideas

- **Migration única:** `supabase/migrations/20260808000001_f38_1_create_ai_cost_accounting.sql` — ALTER `generation_events` (colunas D2 + CHECK `cost_source` D4 + CHECK `generation_type` expandido D5 + índices), ALTER `campaigns` (`operation_run_id UUID` + índice, D1/D2), `ai_model_pricing` + seeds + RPC `admin_set_ai_model_price` (D8) + RLS service_role, views `admin_ai_*` + `admin_cost_vs_credits` (D10) + RPC `admin_get_ai_costs` (D10). **Não toca** `reserve_credit`/`credit_transactions` (F24) nem `admin_get_metrics` (F28)
- **Novo módulo `src/lib/ai-cost/types.ts` (sem server-only):** `COST_SOURCES`, `CostSource`, `OPERATION_RUN_TYPES`, `OperationRunType`, `TokenUsage`, `CostResolution`, `AiCostEvent`, `AiCallInfo` (D1/D4/D7/D12)
- **Novo módulo `src/lib/ai-cost/tracker.ts` (server-only):** `AiCostTracker` com `startRun(type)` (gera `operationRunId` UUID + `traceId` distintos) e `record(event)` best-effort (nunca lança; falha logada)
- **Novo módulo `src/lib/ai-cost/ai-model-pricing.ts`:** `DEFAULT_AI_MODEL_PRICING` (bootstrap) + fonte de leitura da tabela (linha vigente por provider+model)
- **Refatorar `src/lib/ai-cost/cost-estimator.ts`:** `estimateAiCost` → `resolveAiCost` por fonte; `CostResolution` com `costSource`/`pricingVersion`; corrige gemini-3.1-flash-lite, gpt-image-2, cached/image tokens
- **Serviços com callback `onCall`:** copy, input validation, image review, VS generator/validator, brand profiler/director/text-only — sem quebrar contratos públicos (opcional, retrocompatível)
- **Rotas instrumentadas:** `generate-image`, `generate-without-logo`, `/brand-profile/*`
- **Admin APIs (sem página):** `GET`/`PUT /api/admin/ai-model-pricing` + `GET /api/admin/ai-costs`
- **Verificação SQL/integrada I1–I6 (obrigatória):** migration em banco real (colunas + CHECKs + índices incl. `campaigns.operation_run_id`), RPC pricing versiona (fecha + abre, p_reason antes dos opcionais), RLS authenticated sem acesso, resolveAiCost com seeds → pricing_table com uuid + seed só imagem persiste, views com dados reais sem duplicar delivery markers, admin_get_metrics segue respondendo
- **Regressões a cobrir:** pipeline 402/409/estorno, VS F29.1.1, gates F32/F33/F34/F36, legal F30, créditos F24/F38
- **Entrega verificável:** `npx vitest run`, `npm run typecheck`, `npm run lint`, `npm run build` — zero erros; UAT local (gerar campanha → eventos call-level + delivery custo NULL; rejeitar/recompor no mesmo run; gerar VS → eventos com custo + falha técnica novo run; gerar brand profile → brand_profile_*; query admin_cost_vs_credits; PUT pricing altera preço → novo pricing_version)
</specifics>
...
- Página admin de preços (`/admin/ai-model-pricing`) — fase curta futura (D8)
- Página/dashboard de reconciliação — só views/RPCs (D10)
- Cobrança dinâmica / ajuste de preço de crédito / margem mínima — F39 (Stripe)
- Tema (`theme_id`) — coluna preparada; instrumentação quando temas existirem
- Deprecação formal de `/api/campaign/generate` (legado)
- Remoção do `MetricsWriter` — mantido para debug local; deixa de ser fonte de custo
- Alteração de `reserve_credit`/`credit_transactions` (F24) — ledger intacto; reconciliação por leitura
- Evoluir `admin_get_metrics` (F28) — mantido compatível
- Reabertura cross-request do `operation_run_id` — mecânica é escopo da F37; nesta fase a coluna nasce preenchida (D1/D2)
- i18n (produto PT-BR)
</deferred>
...
*Context gathered: 2026-08-08 via OpenSpec artifacts*

...
- **Ordem D5 na rota:** pré-gerar `campaignId` → gerar `id` por imagem (uuid) → `uploadCampaignInputImage` ANTES do snapshot → montar snapshot com `storagePath` → `createCampaign(storeId, input, campaignIdPreGerado)` → em falha pré-stream `removeCampaignInputs`

...
- **oauth-auth-callback (ADDED):** rota `/auth/callback` lê `code` → `exchangeCodeForSession(code)` (PKCE); allowlist de `next` (`VALID_NEXT = ["/loja", "/dashboard"]`, fallback `/dashboard`, externo bloqueado, `"/"`/`/onboarding` nunca válidos); erro genérico `/login?error=oauth_failed`; sucesso → `/dashboard` → PrivacyGate (D16)

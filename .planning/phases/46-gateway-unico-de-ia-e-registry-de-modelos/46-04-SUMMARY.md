...
- **4 falhas transicionais em `src/app/api/campaign/generate-image/__tests__/route.test.ts`:** os testes ainda afirmam o `recordCall` manual de `campaign_input_validation`/`campaign_image_review` (removido neste plano). A co-migração dessa suíte está explicitamente atribuída ao **46-05 Task 4** (plano lista o arquivo). As demais 2693/2697 asserções da suíte completa passam.
- **Janela transitória VS:** a rota `visual-signature/generate-without-logo` (46-05) ainda não injeta o sink; o validator opera best-effort (`valid: true`) até a 46-05. Documentado como decisão.
...
- **Onda 5 (46-05) desbloqueada:** as 4 capacidades de visão executam via gateway; resta migrar as 3 de imagem + furos 3/4 + cost-estimator, e co-migrar `generate-image/route.test.ts` (4 falhas transicionais já mapeadas).
...
- **Threading pronto:** `AiImageGenerator.generate`/`identity-art-director.generate` já aceitam `telemetry` — a rota VS (46-05) só precisa injetar o sink buffering.
- Sem blockers.

...
**`campaign_image`, `campaign_image_edit` e `visual_signature_image` passam a executar via gateway; o fallback images.edit vira segunda invoke explícita (dois envelopes), os furos 3/4 são corrigidos, o cost-estimator soma a tool nas duas capacidades e a soma híbrida manual de imagem é eliminada — 273 files / 2712 testes e 4 gates verdes.**
...
5. **Marcador `imageGenerationTool` preservado no adapter (5.9):** o adapter `responses` só criava `usageMeta` quando havia `usage`; uma imagem bem-sucedida **sem usage** chegava ao sink com `imageGenerationTool: false` e o estimador não somava o componente da tool (bloqueio real apontado na revisão). Corrigido: `usageMeta` é **sempre** criado com `imageGenerationTool: usesImageTool` (`providerUsageRaw` só quando houver usage). Teste no `ResponsesAdapter` (imagem presente sem usage → `usageMeta.imageGenerationTool === true`) + **teste integrado adapter → gateway → sink → estimador** (`image-tool-telemetry-integration.test.ts`), que impede a regressão da conexão.
...
Testes adicionados: adapter responses sem imagem → capability; adapter images com usage → normalizado; provider sem imagem + primary → fallback (2 invokes); sem imagem sem primary → propaga; safety/rate-limit não acionam; cost-estimator `campaign_image`/`visual_signature_image` sem usage → componente da tool; adapter responses imagem sem usage → marcador `imageGenerationTool: true`; integração adapter → gateway → sink → estimador.

Gates re-executados: **273 files / 2712 testes — 0 falhas**; typecheck/lint/build verdes.
...
- [x] `npx vitest run` (suíte completa) → **273 files / 2712 testes, 0 falhas**

...
**Status:** PASSED — UAT local aprovado, migration remota aplicada/verificada, deploy de produção concluído (pós-cleanup Ready) e env-vars obsoletas removidas; fluxo feliz validado em produção
...
| `ai-model-selection`: generation flow unchanged | gateway untouched; frozen guard; full regression | PASS | Deploy completed 2026-09-15; production HTTP 200 |
...
| Proposal: migration before deploy | remote migration applied and verified before any deploy | SATISFIED | Deploy completed/verified |
...
- [x] Frozen contract verifier: 63 paths / 0 violations.
...
- [x] Remote env verification: 11 env-vars obsoletas de modelo/provider removidas da Vercel (Production + Preview); chaves (`OPENAI_API_KEY`/`GEMINI_API_KEY`) e operacionais preservadas.
- [x] Post-cleanup deploy: novo deployment Vercel Production **Ready** criado **depois** da remoção das env-vars; produção HTTP 200; sem erro de configuração.
- [x] Production happy path (human-validated 2026-09-15): campanha/arte gerada e aberta normalmente; crédito e telemetria corretos; modelos efetivos conforme os defaults do registry; nenhum erro de configuração.
...
- No remote env var was created or changed; 11 obsolete model/provider env vars were removed on 2026-09-15 after the deploy (explicit `authorize env cleanup`). Chaves e operacionais preservadas. Um novo deployment Vercel Production foi criado depois da limpeza e ficou **Ready**; produção HTTP 200 sem erro de configuração.
- Production happy path validado por humano (2026-09-15): geração de campanha/arte e abertura normais; crédito e telemetria corretos; modelos efetivos conforme defaults do registry; sem erro de configuração.

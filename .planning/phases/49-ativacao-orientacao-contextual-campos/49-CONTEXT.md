...
- `src/components/flow/store-identity-form.tsx` (**2735 linhas**) — `TONE_OF_VOICE_OPTIONS` em `:37` (8 opções), render do select em `:1956-1978`; `Nome da Loja` em `:1628-1634`; atalhos fiscal→nome em `:1607-1622`; card informativo do Posicionamento em `:1949-1954`; `Posicionamento` (placeholder `Ex: A melhor loja de...`) em `:1968`; `Descrição Curta` em `:1972`; `Slogan` em `:1976`. **Sem microcopy** de nome público/tom de voz/distinção.
- `src/components/flow/campaign-input-form.tsx` (**684 linhas**) — seção Produto com `Descrição (opcional)` e placeholder promocional `Ex: 20% OFF em todo o estoque` em `:337-406`; bloco permanente de 3 regras de preço em `:427-432`; `Preço Original (opcional)`/`Preço Final` em `:433-494`; `IntentSelector` em `:242-292`; `ValidityField`; seção Avisos com `IllustrativeNoticeField` (`showIllustrativeNotice` em `:595-596`) + `MandatoryArtworkField` (`mandatoryArtworkTextFree` em `:600-601`).
- `src/components/campaign/mandatory-artwork-field.tsx` (**34 linhas**) — textarea `Texto obrigatório na arte` (`id="mandatoryArtworkText"`, `maxLength={200}`, `rows={2}`) com hint de regras/restrições.
- `src/components/campaign/validity-field.tsx` (237 linhas) — **NÃO é modificado**; entra apenas como regressão.
- `src/components/flow/campaign-brief-review.tsx` (289 linhas) — `buildMandatoryArtworkText(fields.showIllustrativeNotice, fields.mandatoryArtworkTextFree)` em `:69-76`; seção Avisos renderiza o resultado como bloco único em `:227-246`; preços "Preço original"/"Preço final" em `:163-178`; validade em item próprio `:179-184`.
...
- `src/app/(app)/admin/laboratorio/_components/lab-textarea.tsx` (71 linhas) — precedente de `useId` + `aria-describedby` (erro + hint) e `min-h` de toque.
- **Não há primitivo reutilizável de disclosure**; porém **existem** dois usos locais de `<details>/<summary>` (`src/app/(app)/admin/laboratorio/_components/evaluation-form.tsx:292-307` e `src/app/(app)/reviews/page.tsx:192`) e um `<button aria-expanded>` em `src/components/shell/account-menu.tsx:41-60`. A ajuda expansível é um **primitivo novo** (mínimo e acessível), podendo seguir qualquer um dos dois padrões já usados.
- **Não existe utilitário `cn`** no repo — as classes são concatenadas por template literal (ex.: `store-identity-form.tsx:1328-1342`).
...
- **Nenhuma divergência substantiva** entre `design.md` e o código real (arquivos, campos e consumidores verificados em 2026-09-18). **Correções factuais aplicadas ao CONTEXT após o pattern mapping (2026-09-18):** `store-identity-form.tsx` = **2735** linhas (não 2601); `campaign-input-form.tsx` = **684** linhas (não 651); `mandatory-artwork-field.tsx` = **34** linhas; `lab-textarea.tsx` = **71** linhas. O `design.md` afirmava que "não há `<details>`/disclosure no código atual" — na verdade **existem** dois usos locais de `<details>/<summary>` (`evaluation-form.tsx:292-307`, `reviews/page.tsx:192`) e um `<button aria-expanded>` (`account-menu.tsx:41-60`); a decisão D2 (criar um primitivo novo mínimo de ajuda expansível) permanece. Não há utilitário `cn` (classes por template literal). Nenhuma correção afeta o escopo.

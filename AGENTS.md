...
**Status:** EM EXECUÇÃO ◆ — 1/14 plans (9 waves); fonte da verdade `openspec/changes/fase-48-1-laboratorio-ia-minimo/` (proposal / design D1–D18 / 9 specs / tasks 48-1-01..48-1-14); **48-1-01 concluído** (migration local das 8 tabelas `lab_*` + bucket `lab-artifacts` + RLS/grants + triggers de imutabilidade + RPCs `lab_reserve_run`/`lab_create_experiment`; ainda **não** aplicada no remoto — push deliberado no 48-1-14).

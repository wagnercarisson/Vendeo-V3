...
- [x] Phase 38: Tabela de Custos por Operação (8/8 plans ✅)
  ...
  - [ ] **38.2 = Admin de Custos Operacionais + Configurações Econômicas** (desdobramento da F38, v1.5) — painel admin `/admin/ai-operation-costs` (KPIs, filtros, tabela por entrega, drilldown call-level, agregados por segmento), parâmetros econômicos configuráveis (`usd_brl_rate`, `credit_value_brl`), badges de confiança, correção do `/admin/metrics` — **Pending** (planejamento), fonte `openspec/changes/fase-38-2-admin-custos-operacionais/`
...
| 38.2. Admin de Custos Operacionais + Configurações Econômicas | v1.5 | 0/0 | ○ Pending | — |

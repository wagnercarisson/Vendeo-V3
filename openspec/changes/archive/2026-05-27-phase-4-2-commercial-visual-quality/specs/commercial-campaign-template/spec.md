...
The composition background SHALL use colors derived from the store segment palette (`SEGMENT_PALETTES`). The background SHALL include visual treatment beyond a solid color — such as subtle gradient, radial overlay, or texture — as defined in the 4.2.0 contract.

#### Scenario: Segment background with treatment

- **WHEN** the campaign is rendered
- **THEN** the background color SHALL be resolved from `resolveCampaignBackgroundColor(storeSegment)`
- **AND** visual treatment (gradient, overlay) SHALL be applied per the 4.2.0 contract

#### Scenario: Fallback background renders solid

- **WHEN** no segment is matched
- **THEN** the background SHALL fall back to `#FFFFFF`
- **AND** basic visual treatment SHALL still apply as defined by the 4.2.0 fallback rules

### Requirement: Product image treatment

The product image SHALL be integrated into the composition with professional treatment — intelligent contain/crop, shadow or vignette, and natural separation from text zones. The exact treatment SHALL follow the 4.2.0 contract.

#### Scenario: Image renders with treatment

- **WHEN** a valid `productImageUrl` is provided
- **THEN** the image SHALL be displayed with the treatment defined in the 4.2.0 contract
...
The CTA SHALL render as an integrated campaign element — not as a generic disabled UI button. Its styling, position, and visual weight SHALL follow the 4.2.0 contract.
...
- **THEN** the logo SHALL render as a circular element sized according to the 4.2.0 Commercial Art Direction contract above the store name
...
| original_price_display | Only discounted price renders (larger) |
| badge_text | No badge rendered, no empty space |
| subtitle | Description zone hidden, CTA shifts up |
...
The CampaignRenderer SHALL preserve the responsive 1080x1080 container (`w-full max-w-[1080px] aspect-[1/1]`) for browser preview. The internal layout changes but the outer container API stays identical.

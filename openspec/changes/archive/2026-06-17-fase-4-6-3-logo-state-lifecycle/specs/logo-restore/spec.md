...
- **THEN** any currently active assets SHALL be archived first (prevents unique index violation)
...
- **AND** a warning SHALL indicate no brand direction was restored
...
1. Archive any currently active assets (prevents unique index violation)
...
5. The historical profile SHALL NOT be re-activated — it remains with its current status (or outdated if it coincidentally was the active one)

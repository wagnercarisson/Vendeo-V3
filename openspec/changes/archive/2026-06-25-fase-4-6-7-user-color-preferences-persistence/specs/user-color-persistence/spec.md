...
### Requirement: Voltar para cores sugeridas

O sistema SHALL exibir uma ação discreta "Voltar para cores sugeridas" no bloco de cores, visível apenas quando `hasUserChosenColors` for `true`.

Comportamento ao clicar depende da existência de synced profile:
...
3. Limpa `brand_colors_chosen` no estado local
4. UI passa a exibir `safe_color_tokens` ou fallback equivalente, se existir
5. Nenhum asset visual é alterado (logo, VS, direção visual, campanha)

#### Scenario: Botão visível apenas com escolha manual ativa
...
- **AND** `brand_colors_chosen` SHALL ser `[]`
- **AND** UI SHALL exibir as cores sugeridas pelo Vendeo, se existirem

#### Scenario: Reset sem synced profile limpa estado local

- **WHEN** o usuário clica em "Voltar para cores sugeridas"
- **AND** não existe synced profile
- **THEN** `brand_colors_chosen` local SHALL ser `[]`

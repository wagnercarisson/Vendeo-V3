...
> Part of `fase-14-integracao-fluxo-geracao` (ADDED).

## Purpose

Serviço de transcodificação de imagem (dataUrl → JPEG sRGB q90 1080×1080 via sharp) e builder de publication copy snapshot no shape da milestone v1.3 (caption, hashtags, cta_post). Conecta `dataUrlToCampaignImage` (F13) a `uploadCampaignImage` (F13), garantindo que o formato canônico JPEG seja respeitado.

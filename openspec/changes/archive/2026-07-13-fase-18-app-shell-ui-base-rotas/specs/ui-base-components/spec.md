...
The system SHALL provide a `Button` component at `src/components/ui/button.tsx` with three variants:
...
- `disabled` attribute and reduced opacity styling when disabled
- `loading` state: spinner icon + disabled behavior
...
- `label` prop rendered as `<label>` above the input
- `error` prop for inline validation message (red text below input)
- `...input` spread for native `<input>` attributes (placeholder, type, onChange, value, etc.)
- Styled with design tokens (`bg-bg-surface`, `text-text-primary`, `border-border`, focus `ring-accent-green`)
...
- **AND** the error text SHALL use `text-accent-red` or equivalent error color
...
- `rounded` prop: border-radius value (e.g., `"md"`, `"full"`, `"xl"`)

...
- [x] 1.1 Criar migration `supabase/migrations/20260718000001_create_admin_tables.sql` com:
...
- [x] 2.1 Adicionar `admin_grant_credits()` RPC:
...
- [x] 2.2 Adicionar `admin_create_store_for_user()` RPC:
...
- [x] 3.1 Criar `src/lib/admin/require-admin.ts`:
...
- [x] 4.1 Modificar `src/middleware.ts`:
...
- [x] 5.1 Criar `src/app/(app)/admin/layout.tsx`:
...
- [x] 6.1 Criar `src/app/(app)/admin/page.tsx` — Dashboard operacional com visão geral rápida (total de usuários, campanhas com erro, últimas ações do audit log)
- [x] 6.2 Criar `src/app/(app)/admin/users/page.tsx` — Diretório Server Component com:
...
- [x] 6.3 Criar `src/app/(app)/admin/users/[id]/page.tsx` — Detalhe Server Component com:
...
- [x] 6.4 Criar `src/app/(app)/admin/campaigns/errors/page.tsx` — Lista de campanhas com erro (Server Component + paginação)
- [x] 6.5 Criar `src/app/(app)/admin/audit-log/page.tsx` — Histórico de ações administrativas (Server Component + paginação + filtros)

## 7. Store Creation (Convite Beta)

- [x] 7.0 Criar spec `specs/admin-store-create/spec.md` — criação de loja pelo admin via RPC wrapper atômico `admin_create_store_for_user`
- [x] 7.1 Criar `src/app/api/admin/stores/route.ts`:
...
- [x] 8.1 Criar `src/app/api/admin/credits/grant/route.ts`:
...
- [x] 10.1 Criar testes de integração para `POST /api/admin/credits/grant`:
...
- [x] 11.1 Criar testes para `POST /api/admin/stores`:
...
- [x] 12.1 Criar testes para `admin_audit_log`:
...
- [x] 13.1 Criar testes para endpoints de listagem:
...
- [x] 14.1 Executar `npx vitest run src/lib/admin/__tests__/` — 3+ testes passando
- [x] 14.2 Executar `npx vitest run src/app/api/admin/__tests__/` — 25 testes passando (requer 20+)
- [x] 14.3 Executar `npm run typecheck` — zero erros
- [x] 14.4 Executar `npm run lint` — zero erros
- [x] 14.5 Executar `npx vitest run` — 832 passando (zero regressão)
- [x] 14.6 Executar `npm run build` — build bem-sucedido
- [x] 14.7 Verificar UAT local: admin cria loja para usuário sem loja → loja criada com saldo inicial
- [x] 14.8 Verificar UAT local: admin concede crédito → saldo atualizado → audit log registrado

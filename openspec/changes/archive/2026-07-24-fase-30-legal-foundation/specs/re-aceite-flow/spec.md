...
- The system SHALL block access to: campaign generation, visual signature creation, commercial export, any Vendeo engine creation/publication
...
- Update `privacy_acknowledgements` when user dismisses the banner as acknowledgement of the new version
...
When a user re-accepts a new version, the old acceptance records SHALL NOT be deleted. The UNIQUE(store_id, accepted_by_user_id, document_type, document_version) constraint ensures each user-version pair is recorded once.

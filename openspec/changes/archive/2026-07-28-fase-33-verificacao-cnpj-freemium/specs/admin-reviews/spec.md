...
### Requirement: Card de verificação em /admin/users/[id]

O sistema SHALL adicionar card "Verificação Cadastral" na página de detalhe do usuário.

#### Scenario: Card exibe status e dados oficiais

- **WHEN** admin acessa `/admin/users/[id]`
- **THEN** card exibe: `verification_status`, dados oficiais (razão social, endereço, CNAE), botão "Revelar CNPJ", botão de reprocessar (se DEFER)

#### Scenario: Filtro por verification_status na listagem

- **WHEN** admin acessa `/admin/users`
- **THEN** lista inclui coluna `verification_status` e permite filtrar por status

### Requirement: Audit trail de todas as ações

O sistema SHALL registrar todas as ações admin na tabela `admin_audit_log` (padrão F30/F26).

#### Scenario: Todas as ações são auditadas

- **WHEN** admin realiza approve, reject, exception, reveal-cnpj, create-test-store
- **THEN** cada ação é registrada em `admin_audit_log` com `actor_id`, `action`, `target_type`, `target_id`, `metadata`, `created_at`

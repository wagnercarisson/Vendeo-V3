...
- [ ] 1.1 Criar `src/lib/campaign/metrics.ts` com `"server-only"`, `countCampaigns`, `countReadyCampaigns`, `getCampaignSuccessRate`, `getRecentCampaigns`, `RecentCampaignItem` — conforme D1 do design.md
- [ ] 1.2 Reescrever `src/lib/onboarding/count.ts` como reexport de `@/lib/campaign/metrics` — mantendo compatibilidade F19 (D2)
- [ ] 1.3 Verificar que `getUserOnboardingState` em `state.ts` continua funcionando com o reexport (nenhuma alteração em state.ts)
- [ ] 1.4 Verificar que `DASHBOARD_PLACEHOLDER` em `microcopy.ts` não é mais referenciado pelo dashboard; manter constante se zero referências residuais (não remover se quebrar build)
...
- [ ] 2.3 Implementar 3 cards de métricas (`<Card>` da F18) em grid responsivo:
...
- [ ] 2.5 Implementar card de próximo passo adaptativo (`<Card>` da F18):
...
- [ ] 2.6 Adicionar link discreto "Configurar loja" → `/loja` no dashboard
- [ ] 2.7 Rodar `npm run typecheck`, `npm run lint` — zero erros

## 3. Plano 20-03 — Testes e Acabamento Responsivo

- [ ] 3.1 Criar testes de renderização do dashboard em `src/__tests__/app/dashboard/dashboard-page.test.tsx`:
...
- [ ] 3.4 Rodar `npm run typecheck`, `npm run lint` — zero erros

## 4. Verificação Final

- [ ] 4.1 Dashboard real visível para `has_store_with_campaigns` com saudação, métricas, recentes, next-step, links
- [ ] 4.2 Estados vazios da F19 preservados: `no_store` e `has_store_no_campaigns`
- [ ] 4.3 `countCampaigns` importado de `@/lib/onboarding/count` ainda funciona (compatibilidade F19)
- [ ] 4.4 Nenhuma signed URL gerada no dashboard
- [ ] 4.5 `DASHBOARD_PLACEHOLDER` não é mais referenciado pelo dashboard (pode permanecer em microcopy.ts)
- [ ] 4.6 Rodar `npm run typecheck` — zero erros
- [ ] 4.7 Rodar `npm run lint` — zero erros
- [ ] 4.8 Rodar `npx vitest run` — todos os testes passando (15-20 novos + existentes)
- [ ] 4.9 Rodar `npm run build` — build bem-sucedido

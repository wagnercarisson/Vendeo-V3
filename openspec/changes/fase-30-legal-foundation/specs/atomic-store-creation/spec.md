...
BEGIN
  ...
  -- 4. Concede créditos iniciais (6 parâmetros: store_id, amount, reason, idempotency_key, metadata, type)
...
4. Call `grant_credits(v_store_id, p_initial_grant_amount, 'onboarding', 'onboarding_' || v_store_id, '{}'::jsonb, 'bonus_onboarding')` — 6 positional args matching the existing function signature (p_store_id, p_amount, p_reason, p_idempotency_key, p_metadata, p_type)
5. Return `{ store_id: ... }`
...
- **WHEN** any step of the RPC fails (e.g., duplicate store name)
- **THEN** the entire transaction SHALL be rolled back
- **AND** no partial state SHALL exist (no store without acceptance, no acceptance without store)

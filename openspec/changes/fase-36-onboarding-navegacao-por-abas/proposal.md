...
- **Desbloqueio progressivo (soft-block)** — abas nascem bloqueadas e liberam quando a anterior tiver o mínimo válido; o usuário nunca vê tela em branco — vê a aba bloqueada com o motivo no painel ativo
...
- **Estados por aba simplificados** — `Bloqueada / Rascunho / Salva / Pronta / Pendente para gerar` via `computeTabState` puro; motivo específico no painel ativo (não no botão da aba, principalmente no mobile)
- **Abas mobile compactas** — labels `Dados / Perfil / Visual` (apenas label responsivo; o `id` permanece `posicionamento`/`direcao-visual`), badge pequeno, motivo fora do botão, botão inferior "Continuar" sempre visível
...
- `store-onboarding-tabs`: Máquina de abas do onboarding — `tabs.ts` (definição das 3 abas + `computeTabUnlock` retornando `{ unlocked, reason }`), `tab-state.ts` (`computeTabState` puro) e componente `store-tabs.tsx` (container ARIA tabs + variante mobile compacta). Cobre desbloqueio progressivo (D1/D2), URL `?tab=` (D6), estados por aba (D7), regra do tom de voz (D9), mobile compacto (D10) e ARIA (D11)

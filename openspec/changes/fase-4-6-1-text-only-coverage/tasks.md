...
- [ ] 1.1 Create `supabase/migrations/20260612000001_add_identity_state_fields.sql` with:
...
- [ ] 1.2 Apply migration locally and verify `stores` and `store_brand_profiles` schemas

## 2. Types / Interfaces

- [ ] 2.1 Update `Store` interface to include fields from migration: `identity_state`, `text_only_origin`, `manual_color_override`, `previous_identity_snapshot`
- [ ] 2.2 Update `BrandProfileRecord` type to include `manual_color_override` field
- [ ] 2.3 Define `TextOnlyInferenceInput` and `TextOnlyInferenceResult` types matching `BrandProfilerWithoutLogoResult` shape (safe_color_tokens, visual_style, visual_tone, typography_direction, brand_personality, campaign_guidelines, campaign_brief, inferred_primary_color, inferred_accent_color, confidence_score)

## 3. Brand Inference Prompt

- [ ] 3.1 Create `prompts/store-brand-inference.md` — prompt for AI to infer visual identity from store data alone (name, segment, subsegment, tone_of_voice, positioning, short_description, slogan, city, state) with optional user color preferences as signal
- [ ] 3.2 Verify prompt is loadable via `PromptLoader`

## 4. BrandTextOnlyInferenceService

- [ ] 4.1 Create `src/lib/brand-assets/text-only-inference-service.ts` — service loads prompt, fills template, calls OpenAI, validates output, returns `TextOnlyInferenceResult`. Does NOT persist or update store state (route handles that). Throws on API key missing in production; returns mock in development.

## 5. API Routes

- [ ] 5.1 Create `src/app/api/store/[id]/brand-profile/infer/route.ts`:
...
- [ ] 5.2 Update `PATCH /api/store/[id]/brand-profile` to handle color changes:
...
- [ ] 6.1 Update `src/lib/actions/store.ts` — `resolveStoreIdentity`:
...
- [ ] 7.1 Update `src/components/flow/store-identity-form.tsx`:
  ...
  - Remove "Continuar sem logo" link when `identity_state='text_only'` (show both buttons + chip if synced, or retry if failed)
...
- [ ] 7.2 Update `src/components/flow/store-preview.tsx`:
...
- [ ] 8.1 Run TypeScript check (`npx tsc --noEmit`) and fix type errors
- [ ] 8.2 Run linter and fix any issues
- [ ] 8.3 Run build (`npm run build`) and verify it succeeds
- [ ] 8.4 Manual flow test: create store → Step 2 → click "Continuar sem logo" → verify inference trigger, spinner, result in UI, profile in DB
- [ ] 8.5 Manual flow test: create store → Step 2 → Save without logo (with and without colors) → verify inference and dual-population
- [ ] 8.6 Manual failure test: simulate API error (e.g., invalid API key in production mode, or mock service to throw) → trigger inference → verify non-blocking 200, profile `failed`, stores updated, retry button shown

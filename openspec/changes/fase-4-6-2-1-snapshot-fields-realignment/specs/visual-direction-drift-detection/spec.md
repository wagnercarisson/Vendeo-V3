...
**Reason**: O conceito de "sensitive field set" único (6 campos, incluindo cores) foi substituído por dois conjuntos separados: `SNAPSHOT_FIELDS` (captura de 7 campos textuais) e `DRIFT_FIELDS` (política de drift de 4 campos). Cores (`brand_color`, `accent_color`) são dados derivados e deixam de ser capturadas ou comparadas.

**Migration**: `SENSITIVE_FIELDS` é removido de `drift.ts`. Usar `SNAPSHOT_FIELDS` para estrutura de snapshot e `DRIFT_FIELDS` para comparação de drift.

...
- Compensação no backend do realinhamento (POST /brand-profile/realign): inferência executada ANTES de qualquer mutação no banco; perfil anterior marcado como `outdated` APENAS após inferência bem-sucedida; novo perfil inserido com `status = 'synced'`; se o insert falhar, perfil anterior restaurado para `synced`. **Nunca podem existir dois registros `synced` simultâneos.** Inferência falha: perfil anterior NÃO marcado como outdated (permanece `synced`)

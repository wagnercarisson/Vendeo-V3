...
- Oferecer realinhamento sob demanda (re-inferência in-place) sem navegação forçada
- Interromper com modal apenas em ação de alto impacto (Gerar Campanha), onde o desalinhamento afeta o resultado final

**Non-Goals:**
- Transições entre identity_state (text_only ↔ logo ↔ visual_signature) — escopo de 4.6.x
- Dashboard notifications — escopo futuro
- `positioning` como campo sensível — fora de escopo, tratado em ocasião futura se necessário
- Export action modal — o componente de exportação ainda não existe; o modal nesta fase cobre apenas "Gerar Campanha"
...
### D6 — UX: banner → botão discreto, sem modais em salvar/sair
...
| **Usuário ignora drift e esquece**: botão discreto pode passar despercebido | Botão discreto é o suficiente para esta fase. Revisão UI/UX antes de produção pode reposicionar |

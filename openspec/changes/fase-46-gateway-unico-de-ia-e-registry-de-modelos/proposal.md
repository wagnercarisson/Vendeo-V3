...
- **Interface `AiModelResolver`** (`src/lib/ai/model-resolver.ts`): seam de resolução de modelo, **assíncrono** (`resolve(capability): Promise<AiModelConfig>`, com `AiCapability`). O registry em código é a implementação inicial (`async resolve()`); o Change B poderá decorar/substituir o resolver (seleção persistida e cacheada) **sem refazer o gateway** — o `AiGateway` recebe `(resolver, adapters)` por construtor.
...
- **Código novo**: `src/lib/ai/model-registry.ts`, `src/lib/ai/model-resolver.ts` (`AiCapability` + interface `AiModelResolver` assíncrona), `src/lib/ai/gateway.ts` (`AiGateway(resolver, adapters)`), `src/lib/ai/types.ts` (`AiCallEnvelope`, `AiTelemetryContext` com `sink` obrigatório), `src/lib/ai/telemetry-sink.ts`, `src/lib/ai/generation-type-map.ts`, `src/lib/ai/adapters/{chat-completions,responses,images,gemini}.ts`, `src/lib/ai/adapters/registry.ts`, `src/lib/ai/index.ts` (composição) e `src/lib/ai/api-keys.ts` (definido em `design.md`). A interface `ImageProvider` (`image-generation/providers/types.ts`) é **mantida** como contrato de domínio/seam de testes, com implementação delegando ao gateway.

...
## 2. Plan 46-02 — Gateway, contrato e adapters (onda 1)
...
- [ ] 2.4 Estender `AiCallInfo` (`src/lib/ai-cost/types.ts`) com `capability`, `protocol`, `status` e `errorType`; testes: envelope por tentativa (success/failed/timeout), contrato de usage normalizado, ausência de usage explícita, sem retry/fallback, propagação de signal, modelo real
...
## 3. Plan 46-03 — Migração das capacidades de TEXTO (onda 2)
...
- [ ] 3.4 Migrar `campaign_spec` legado (`campaign-intelligence/providers/openai.ts`) para o gateway usando o literal `campaign_spec`; o fallback `json_schema`→`json_object` é segunda `invoke` explícita do orquestrador
- [ ] 3.5 Rodar gates e co-migrar testes de texto afetados (sem mudança de comportamento)

## 4. Plan 46-04 — Migração das capacidades de VISÃO (onda 2)
...
## 5. Plan 46-05 — Migração das capacidades de IMAGEM (onda 2)
...
## 6. Plan 46-06 — Cobertura de telemetria em todos os callers produtivos sem onCall (onda 3)
...
- [ ] 6.5 Teste de inventário: todo `import`/chamada de serviço de IA em rota/action produtiva emite telemetria (nenhum caminho fora da camada única)
- [ ] 6.6 Testes dos caminhos: evento call-level com custo, modelo real e duração; regressão de comportamento

## 7. Plan 46-07 — Remoção das env-vars de modelo (onda 3)
...
## 8. Plan 46-08 — Regressão e não-mudança do contrato externo (onda 4)
...
## 9. Plan 46-09 — Verificação final (onda 5)

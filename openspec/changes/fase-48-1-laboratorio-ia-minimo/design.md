...
- **Congelamento (correção de ajuste)**: `draft` e `ready` têm configuração **editável** (sem runs); a partir do **primeiro run** (`running`) a configuração é **congelada** — o trigger de banco consulta o estado do experimento e bloqueia UPDATE das variantes/alvo/params somente depois do primeiro run. Editar após o primeiro run exige criar um novo experimento. Na F48.1 essa editabilidade é **apenas** uma propriedade de banco/domínio: a API/UI administrativa oferece **criação e leitura** (sem endpoint/ação de update ou arquivamento); alterar a configuração exige **criar outro experimento**.
...
- **Navegação preservada e decisão não migra (correções de bloqueio)**: os seletores de cenário/repetição SHALL permanecer visíveis mesmo quando a seleção corrente não tem o par comparável (o estado vazio aparece apenas na área dos painéis); o seletor de repetição SHALL oferecer apenas repetições com o par completo (baseline + candidata); o formulário de avaliação SHALL ser reiniciado quando o par comparado muda (via `key` = cenário + os dois run ids), para que uma decisão tomada olhando um par nunca seja registrada em outro; `blind_order` SHALL ser registrado **apenas** quando a escolha acontece efetivamente em modo cego (ligado e não revelado) — caso contrário `null`/omitido.
...
- **Transições compare-and-set (correção de bloqueio)**: `markRunRunning` (`pending → running`) e `finalizeLabRun` (`pending|running → terminal`) SHALL filtrar pelo estado de origem **e** exigir exatamente **1 linha afetada** (`.select("id")`) — um update de zero linhas não pode ser tratado como sucesso e um run já terminal (ex.: reconciliado como órfão) nunca é sobrescrito em nenhuma direção. A reconciliação preguiçosa SHALL **reaplicar atomicamente no próprio update** o estado de origem **e** o cutoff de `coalesce(started_at, created_at)` (ramos mutuamente exclusivos: `running` ⇒ `started_at < cutoff`; `pending` ⇒ `started_at IS NULL AND created_at < cutoff`) e **contar apenas as linhas efetivamente alteradas** — um run que terminou entre o select e o update é excluído pelo filtro e um run promovido a `running` com `started_at` recente entre o select e o update **não** é reconciliado (o executor continua dono do run, sem liberar o índice global por baixo de uma chamada paga em andamento).
- **Evidência preservada na falha (correção de bloqueio)**: a finalização `failed` SHALL persistir as evidências já existentes no sink (`calls`, `usage`, `cost_detail`, `estimated_cost_usd`, `provider`/`model`/`protocol`, `attempts` e `technical_validation` quando calculada) — uma chamada paga que falhou (provider, imagem ausente, artefato) continua auditável. `attempts` só é sobrescrito quando houve chamada real.
- **MIME real do artefato (correção de bloqueio)**: o MIME SHALL ser derivado dos **bytes** pela validação técnica e restrito à allowlist do bucket; bytes indecodificáveis ou com MIME fora da allowlist são recusados com `unsupported_artifact_mime_type` **sem persistir**. MIME + dimensões entram numa **única** operação de persistência — sem update separado e sem erro de update ignorado.
- **Stream NDJSON sanitizado (correção de bloqueio)**: a mensagem de erro SHALL ser sanitizada **uma única vez** na origem e a mesma string segura SHALL ir para o banco e para o evento do stream — nenhum token/URL vaza no NDJSON administrativo.
...
- `ready|running|evaluated → archived`: arquivamento manual (capacidade de banco/domínio; **não exposta pela API/UI da F48.1** — fica para a F48.2).

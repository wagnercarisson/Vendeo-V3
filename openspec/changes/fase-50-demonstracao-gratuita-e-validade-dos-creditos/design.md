...
- Substitui a concessão `bonus_onboarding` nos pontos: `create_store_with_cnpj`, `admin_approve_store_verification`, `admin_exception_store_verification` — todos recebem `p_demo_grant_enabled` do caller. `update_store_cnpj` **passa a conceder a demo** quando a verificação resulta em `approved`, a flag está ativa e a raiz não tem benefício anterior (D8), e **só** cria o marcador legado quando há evidência real de benefício anterior.

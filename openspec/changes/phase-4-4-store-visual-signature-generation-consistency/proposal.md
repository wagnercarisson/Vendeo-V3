...
- **Geração de assinatura visual leve**: quando não há logotipo, o sistema gera até 3 variações de assinatura visual simples (símbolo + nome da loja) via IA, usando dados da loja (nome, segmento, cor principal, tom de voz).
...
- **Fluxo pós-salvamento da loja**: modal ou etapa guiada perguntando se o lojista quer escolher uma assinatura ou deixar o Vendeo criar automaticamente.
- **Fallback tipográfico persistido**: se a geração por IA falhar, o sistema gera uma assinatura tipográfica simples e a persiste como asset reutilizável (Storage), não apenas preview temporário.
...
- **Novo serviço**: `src/lib/visual-signature/service.ts` para geração de assinatura visual via IA.
- **Novos prompts**: `prompts/visual-signature-generator.md` para gerar descrições de assinaturas visuais.

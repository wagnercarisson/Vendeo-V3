...
Controlar acesso administrativo ao console operacional do Vendeo via tabela `admin_users` + função `requireAdmin()`. Sem flag em `auth.users`. A proteção combina middleware (sessão) + server component/API gate (admin_users).

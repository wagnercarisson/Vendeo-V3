...
Esta spec define o backend de inteligência de campanha — o serviço que transforma dados de produto + oferta + loja em um `CampaignSpec` estruturado e validado via inferência de IA, com interface abstrata de provider e validação Zod. A capacidade legada `campaign_spec` passa a ser resolvida pelo registry e executada via gateway (`fase-46-gateway-unico-de-ia-e-registry-de-modelos`).
...
The system SHALL provide an `OpenAIProvider` class at `src/lib/campaign-intelligence/providers/openai.ts` that implements `AIProvider`.

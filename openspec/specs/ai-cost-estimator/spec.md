...
> Synced from `fase-28-observabilidade-operacao-launch-controls` (ADDED), then `fase-38-1-ai-cost-accounting` (MODIFIED + ADDED). `estimateAiCost` vira o resolvedor `resolveAiCost` (provider_reported → pricing_table → fallback_static → not_available); adicionado componente provisório versionável da tool `image_generation` (fórmula v2) e o requisito `Modelo desconhecido → null (legado)`.

## Purpose

Helper para estimar custo de chamadas a APIs de IA (OpenAI, Gemini) com base em tabela de preços interna, permitindo registrar custo estimado em telemetria.

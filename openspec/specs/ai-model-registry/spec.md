...
> Updated by `fase-47-catalogo-e-selecao-de-modelos-admin` (ADDED — resolução com override persistido e paridade registry × catálogo).

## Purpose

Registro central de modelos por capacidade/segmento, resolução de chaves de API por provider e eliminação das env-vars de modelo como mecanismo de configuração. O registry em código (`src/lib/ai/model-registry.ts`) é a única fonte da verdade da escolha de modelo; a interface `AiModelResolver` é o seam que permite a seleção persistida do Change B (F47) sem reabrir o gateway.

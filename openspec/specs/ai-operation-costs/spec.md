...
> Synced from `fase-38-2-admin-custos-operacionais` (ADDED).

## Purpose

Painel admin `/admin/ai-operation-costs` ("Custos de Operação") para análise operacional do consumo de IA por entrega: KPIs, tabela por entrega com badges de confiança, drilldown call-level e agregados — incluindo segmentação econômica (origem operacional do consumo) e custo por tipo/etapa/provider. Consome os contratos de `ai-operation-runs-api`; nunca calcula KPIs sobre a página.

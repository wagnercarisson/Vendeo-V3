...
Define a montagem do **briefing contextual** do diretor de arte de imagem: estrutura dos 4 `.md` por intent, blocos condicionais de seção, regras de presença/ausência (sem seções vazias e sem placeholders residuais), separação semântica entre naturezas de texto, preservação de identidade visual e fidelidade visual das referências.

> Capability nova (ADDED) pela `fase-45-briefing-contextual-do-diretor-de-arte` (Briefing Contextual do Diretor de Arte, v1.5 — concluída em 2026-09-05, 8/8 plans).
...
> Ajustes aprovados em revisão humana pós-fase (45-08, alinhamento Diretor × Revisor — concluído 2026-09-05): consolidação do bloco canônico de identidade (`identityReferenceSection` — área segura, margem nas 4 bordas, posição secundária, fidelidade/anti-invenção; `text_only` → cores/linguagem como referência sem inventar logotipo/assinatura) com **concordância de gênero** parametrizada por estado (logotipo masculino / assinatura visual feminino). O 45-08 é um adendo à F45 (mesmo change), não reabre a numeração.
...
> Revisão humana (45-06a/45-06b e 45-08): badge informado → incorporação obrigatória no offer (sem badge → não inventar selo/benefício/promessa); CTA não repete literalmente o texto do badge; texto obrigatório multilinha com separação visual/respiro (comportamento comum por conteúdo); regra flat rígida substituída por composição limpa/profissional (luz/sombra natural permitida) sem enfraquecer clareza promocional.
...
> Consolidado no 45-08 (revisão humana): o bloco canônico de identidade orienta manter o ativo **integralmente dentro da área segura da arte, com margem visível nas quatro bordas**, posição livre desde que legível/reconhecível/**secundária ao conteúdo principal**, e aplica **concordância de gênero** por estado (`logotipo` masculino / `assinatura visual` feminino). Para `text_only`, cores/linguagem da marca servem de referência sem inventar logotipo/assinatura.
...
> Ajuste autorizado em revisão humana (45-08, commit separado `1cb8a264`): para campanhas de **offer**, o diretor isola o produto do cenário original e o apresenta em **fundo comercial limpo** (fundo criado livremente, sem manter o ambiente contextual da imagem de referência) — sem reduzir a fidelidade factual do produto.
...
> O 45-08 (adendo à F45, aprovado em revisão humana 2026-09-05) **abriu a exceção prevista** para o revisor com necessidade concreta comprovada (defeito de alinhamento Diretor × Revisor descoberto no UAT do 45-07): o contrato interno do `ImageReviewService` foi splitado no mesmo `splitDirectorLegalText` do Diretor e o `campaign-image-reviewer.md` foi reescrito com autoridade estreita e políticas legíveis — **sem** mudar UI/form, contrato HTTP público, schema público, snapshot/domínio, Copy Director ou fallback OpenAI (regressão de não-mudança verificada por git).

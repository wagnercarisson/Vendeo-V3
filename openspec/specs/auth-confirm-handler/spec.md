...
Route handler `/auth/confirm` que processa tokens de confirmação de email via `verifyOtp()` com `token_hash`. Usa allowlist para `next` parameter (previne open redirect). Não usa PKCE/code exchange.

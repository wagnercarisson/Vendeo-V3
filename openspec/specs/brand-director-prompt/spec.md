> **Purpose**: Defines the Store Brand Director prompt used for AI-powered visual analysis of store logos. The prompt instructs the LLM to analyze the logo image, extract colors and visual characteristics, cross-reference with store metadata, and generate a structured brand profile with campaign guidelines.

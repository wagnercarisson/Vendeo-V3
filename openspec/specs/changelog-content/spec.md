> **Propósito**: Fonte de dados do changelog como arquivos Markdown com frontmatter YAML em `content/changelog/`, parser próprio de frontmatter, renderer controlado de markdown (h2/p/ul/li/strong sanitizado), schema Zod fail-fast no build e módulo server-only de leitura/validação/ordenação. Datas tratadas como data civil sem distorção de fuso.
>
> > Created from `fase-35-changelog-novidades` (ADDED).

## Purpose

O changelog editorial do Vendeo é 100% estático: dados em `content/changelog/*.md` com frontmatter YAML, estado de leitura em localStorage (ver `changelog-state`) e parse no server component. Sem Supabase, sem requisição extra, sem dependências novas (parser próprio + Zod já presente). O seed inicial contém 3 entries de exemplo (F30, F32, F34); após o dogfooding da fase, o diretório passa a conter também a entry F35 como a mais recente e o anúncio ativo.

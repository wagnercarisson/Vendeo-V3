...
O sistema SHALL prover `hashCnpjRoot(root: string): string` que calcula HMAC-SHA256 dos 8 primeiros dígitos com pepper server-side. O pepper nunca está no código. Esta função é de uso restrito — na produção, o hash é calculado na rota Next.js (server-side) com `process.env.CNPJ_PEPPER`. `hashCnpjRoot()` é usada em testes e em contexto de servidor.
...
O sistema SHALL prover `maskCnpj(normalized: string): string` que mascara o CNPJ normalizado (14 dígitos) no formato `**.***.***/YYYY-**`. Os 8 primeiros dígitos e os 2 dígitos verificadores são substituídos por `*`. O sufixo YYYY é preservado.
...
O sistema SHALL prover `compareBusinessName(name: string, razaoSocial: string, nomeFantasia?: string): CnpjValidationScore` que calcula similaridade textual (Levenshtein) e retorna score. A validação cadastral é score, não bloqueio — o fluxo nunca é interrompido.

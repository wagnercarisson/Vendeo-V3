...
SQL functions atômicas para mutações de saldo no ledger financeiro: `grant_credits`, `reserve_credit`, e `refund_credit`. Todas usam `SELECT ... FOR UPDATE` para atomicidade e possuem idempotência via partial unique index.

...
> Synced from `fase-38-2-1-economic-snapshot` (ADDED).

## Purpose

Snapshot econômico por evento em `generation_events`: persiste, no momento da geração, os valores vigentes de `usd_brl_rate` e `credit_value_brl` (mais origens), garantindo estabilidade temporal dos derivados (custo BRL, receita estimada, resultado/margem estimados). Inclui contrato de nomenclatura estimada, fallback legacy explícito para eventos sem snapshot, exposição dos snapshots nos RPCs de operation runs e backfill aproximado na migration (via audit, com origem marcada).

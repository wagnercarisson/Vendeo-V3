...
Fluxo de autenticação "Continuar com Google" (`google-button.tsx`) via `supabase.auth.signInWithOAuth`, com visibilidade controlada pela flag `VENDEO_PUBLIC_SIGNUP_ENABLED`, escopos mínimos, sem confirmação de email adicional e sem Turnstile (D3/D15).

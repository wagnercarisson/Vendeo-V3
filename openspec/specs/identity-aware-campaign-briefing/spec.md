...
Defines the identity-aware campaign briefing layer: `ResolvedCampaignContext` internal type (renamed from `CampaignBrief`), `resolveStoreIdentity` declarative pipeline, `validateIdentityReference` accessibility check, and `buildCampaignBrief` directive derivation. This layer connects store identity resolution to image generation with full awareness of identity state, asset availability, and brand profile context.

> Renamed by `fase-39-brief-estruturado-campanha` (D4): o wrapper de transporte passa a chamar-se `ResolvedCampaignContext`; o nome `CampaignBrief` agora designa **o contrato de domínio estruturado** (`src/lib/campaign/brief.ts`).

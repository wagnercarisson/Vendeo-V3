...
> Added by `fase-31-3-quality-gate-por-intencao-comercial`.

## Purpose

Define como o `ImageReviewService` recebe `campaignIntent`, monta variáveis contextuais no prompt, e avalia a imagem contra expectativas específicas de `offer`, `spotlight` e `exclusive`. Inclui o novo tipo de issue `commercial_tone_mismatch` e o tipo union `ReviewIssueType`.

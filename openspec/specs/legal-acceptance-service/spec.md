> **Propósito**: Registro, consulta de status e histórico de aceites contratuais de Termos de Uso e Uso Aceitável (store-level). F42 (D12): reaceite de Termos v1.4 com tolerância técnica; ciência de privacidade (user-level) separada do aceite contratual (store-level).
...
RLS SHALL be enabled. INSERT via service role (server-only). SELECT for own store via RLS.
...
- Ignores unique violation (23505) — re-acceptance is idempotent
...
The system SHALL provide `registerAllContractAcceptances(params)` as a convenience that registers both documents in sequence.
...
### Requirement: Re-aceite de Termos de Uso v1.2 (ADDED)
...
### Requirement: getAcceptanceStatus reconhece v1.2 (ADDED)

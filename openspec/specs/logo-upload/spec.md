...
9. Insert new profile with `status = 'failed'`, `source = 'logo_analysis'`, `active_logo_asset_id = originalAsset.id`, `metadata.attempt_snapshot` populated with the 7 snapshot fields via `buildStoreProfileInputSnapshot(store)`

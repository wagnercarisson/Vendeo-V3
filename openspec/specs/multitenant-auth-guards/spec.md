...
Centralized auth guard infrastructure: `AuthorizedStoreContext` type, `requireAuthorizedStore()` combining auth + ownership, `requireSameOrigin()` CSRF validation, `JsonErrorResponse` helpers, and centralized error classes.

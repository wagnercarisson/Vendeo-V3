...
Rota `/auth/callback` para o fluxo OAuth (Google) via `exchangeCodeForSession` (PKCE), separada do `/auth/confirm` (email/OTP). Allowlist de `next`, fallback seguro para rota protegida, PrivacyGate reusado no layout `(app)` e identity linking automático por email verificado (D16).

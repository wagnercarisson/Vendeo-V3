...
Defines the draft store creation path: the `create_store_draft` RPC, the two-mode `POST /api/store` (draft vs verified/fiscal), the preserved generation/credit gates for draft stores, and the later CNPJ attachment via the existing `update-cnpj` flow.

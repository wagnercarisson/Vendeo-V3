...
Refactor of `useStoreForm` hook and related client components to receive store data from server props instead of localStorage. `save()` determines POST vs PATCH based on local `storeId` state.

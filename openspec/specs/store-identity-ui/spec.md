...
- **CNPJ**: required text input com máscara `XX.XXX.XXX/YYYY-ZZ` (visible ONLY in create mode, required). Dispara consulta assíncrona via `GET /api/cnpj/lookup?cnpj={cnpj}` no evento onBlur (após validação local de dígitos). Exibe loading "Consultando dados cadastrais..." durante lookup.

...
Removes all references to `localStorage("store_id")` from every component in `src/components/flow/` and `src/components/auth/`. Store identity is now resolved server-side via JWT claims.

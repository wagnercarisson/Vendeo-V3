> Created from `fase-36-onboarding-navegacao-por-abas` (ADDED).

## Purpose

Defines the onboarding auto-save orchestration: the silent `autoSave()` in `use-store-form`, fiscal persistence (draft → fiscal) before navigation, the `useOnboardingTabs` hook that coordinates tabs/auto-save/URL, drift preservation during tab navigation, and the per-tab state computation (`computeTabState`).

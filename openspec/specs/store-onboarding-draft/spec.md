...
Defines the persistent onboarding draft stored in `localStorage` with a 24h TTL, scoped per user. Covers the synchronous write on mobile abandonment (`pagehide`/`visibilitychange`), restoration on `/loja` mount and cleanup on logout/first save.

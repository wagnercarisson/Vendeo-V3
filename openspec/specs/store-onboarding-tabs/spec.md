...
Defines the tab-based store onboarding navigation at `/loja` (Dados, Posicionamento, Direção Visual). Includes tab definitions, progressive unlock rules, ARIA tabs container, deep-link handling, back/forward URL state and legacy `?required=` compat.

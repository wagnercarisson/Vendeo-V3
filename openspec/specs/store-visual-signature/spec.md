...
> Modified by `fase-38-credit-operation-costs` (ADDED + MODIFIED). `generate-without-logo` resolves `visual_signature_generation` cost dynamically via `OperationCostService` (D12); VS metadata includes cost snapshot (`operation_key`, `operation_cost_credits`, `operation_cost_source`) alongside `credit_tx_id` (D6).

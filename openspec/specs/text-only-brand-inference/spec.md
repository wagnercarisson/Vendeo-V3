...
3. Have a timeout configurable via `IMAGE_GENERATION_GLOBAL_TIMEOUT_MS` (default 30s)
...
#### Scenario: Timeout returns non-blocking error

- **WHEN** the AI call exceeds the configured timeout
- **THEN** the endpoint SHALL return a non-blocking 200 with `{ success: false, message: "..." }`
- **AND** a brand profile SHALL be persisted with `status: 'failed'`
...
- **WHEN** the OpenAI call exceeds `timeoutMs`
- **THEN** the service SHALL throw a `BrandTextOnlyInferenceError` with `errorType: 'timeout'`
- **AND** the route SHALL handle it as a non-blocking failure (profile `failed`, stores updated)

...
Proteção anti-bot via Cloudflare Turnstile nas telas de signup email/senha, login por senha e recuperação de senha, com integração nativa do Supabase Auth (validação server-side) — nunca no Google OAuth (D3/D15).

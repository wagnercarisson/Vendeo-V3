...
// F46-04 (D9): a rota usa `createDefaultTelemetryContext` de `@/lib/ai`; o sink
...
describe('Brand cost accounting (6.5)', () => {
  ...
  // path 2 simulado: profiler emite 2 envelopes (1a = visão, 2a = texto — ordem fixa documentada)
  function setupSuccessWithTwoCalls() {
    setupStandardStore();
    mockProfilerGenerate.mockImplementation(async (_input: any, telemetry?: any) => {
      await telemetry?.sink?.emit({ capability: 'brand_profile_vision', protocol: 'chat-completions', status: 'success', provider: 'openai', model: 'gpt-4o', usage: VISION_USAGE, durationMs: 300 });
      await telemetry?.sink?.emit({ capability: 'brand_profile_text', protocol: 'chat-completions', status: 'success', provider: 'openai', model: 'gpt-4o-mini', usage: TEXT_USAGE, durationMs: 150 });

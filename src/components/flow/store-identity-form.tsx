...
export function StoreIdentityForm({ initialStore, userId, initialTab, redirectMessage }: { initialStore?: Store | null; userId?: string; initialTab?: OnboardingTab; redirectMessage?: string }) {
  ...
  const [cnpjLookupStatus, setCnpjLookupStatus] = useState<'idle' | 'loading' | 'resolved' | 'not_found' | 'unavailable'>('idle');
  const [cnpjLookupData, setCnpjLookupData] = useState<Record<string, unknown> | null>(null);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
          ...
          <div className="border border-border rounded-lg overflow-hidden">
            <button
              ...
              className="w-full flex items-center justify-between px-4 py-3 text-sm font-heading font-semibold text-text-primary hover:bg-bg-elevated transition-colors"
            >
              <span>Dados para faturamento (opcional)</span>
              <span className={`transform transition-transform ${billingExpanded ? 'rotate-180' : ''}`}>
                ▼
              </span>
            </button>
            {billingExpanded && (
              <div className="px-4 pb-4 space-y-3">
                ...
                {(hasAnyBilling || billingManualActive) ? (
                  <>
                    ...
                    <button
                      type="button"
                      disabled={billingSaving || (!billingData.billing_email && (!billingData.billing_address_street || !billingData.billing_address_number || !billingData.billing_address_city || !billingData.billing_address_state))}
                      onClick={async () => {
                        setBillingSaving(true);
                        try {
                          await fetch("/api/store/billing/confirm", {
                            method: "POST",
                            headers: { "Content-Type": "application/json" },
                            body: JSON.stringify({
                              storeId: storeId,
                      ...
                      className="w-full mt-2 min-h-[38px] rounded-lg bg-accent-green px-4 py-2 text-xs font-semibold text-white hover:brightness-110 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                    ...
                    <div className="space-y-4">
                      <div>
                        <div className="mb-2 flex items-center justify-between">
                          <label htmlFor="tone_of_voice" className="text-text-muted text-xs font-heading font-medium uppercase tracking-wider">Tom de Voz</label>
                          {formData.tone_of_voice !== "" && (
                            <button
                              type="button"
                              onClick={() => setField("tone_of_voice", "")}
                              className="min-h-[44px] text-xs font-body text-text-muted hover:text-text-primary transition-colors duration-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-accent-blue/20"
                            >
                              Limpar seleção
                            </button>
                          )}
                        ...
                        <ToneOfVoiceSelect
                          ...
                          ariaDescribedby={[toneHintId, toneComplementsId].join(" ")}
                        />

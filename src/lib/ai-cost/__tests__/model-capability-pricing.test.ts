...
const target = (capability: string, provider = "openai", model = "gpt-4o", protocol: "chat-completions" | "responses" | "images" = "chat-completions") => ({ capability, primary: { provider: provider as "openai", model, protocol } });
...
describe("capacity-aware model pricing", () => {
  it("faz uma consulta bulk e sinaliza tokens ausentes sem bloquear seleção", async () => {
    ...
    expect(status.fallbackSource).toBe("fallback_static");
  ...
  it("usa bootstrap quando completo e não sinaliza modelo textual", async () => {
    ...
    expect(status.pricingConfigured).toBe(true);

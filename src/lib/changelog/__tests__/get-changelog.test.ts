...
describe("get-changelog", () => {
  it("getAllEntries lê o diretório real e retorna entries ordenadas por data DESC (tom-de-voz 2026-09-19 → F43 2026-08-21 → F40/F41 2026-08-20 → F42 2026-08-19 → F41 2026-08-15 → F40 2026-08-14 → novo-endereço 2026-08-12 → F36 → F35)", async () => {
    const entries = await getAllEntries();

    expect(entries.length).toBeGreaterThanOrEqual(3);
    expect(entries[0].frontmatter.id).toBe("tom-de-voz-seletor-descritivo");
    expect(entries[0].frontmatter.date).toBe("2026-09-19");
    expect(entries[1].frontmatter.id).toBe("fase-43-revisao-brief-pre-geracao");
    expect(entries[1].frontmatter.date).toBe("2026-08-21");
    expect(entries[2].frontmatter.id).toBe("criacao-campanhas-mais-clara-confiavel");
    expect(entries[2].frontmatter.date).toBe("2026-08-20");
    expect(entries[3].frontmatter.id).toBe("fase-42-signup-controlado-elegibilidade-freemium");
    expect(entries[3].frontmatter.date).toBe("2026-08-19");
    expect(entries[4].frontmatter.id).toBe("fase-41-midia-de-campanha-mobile");
    expect(entries[4].frontmatter.date).toBe("2026-08-15");
    expect(entries[5].frontmatter.id).toBe("fase-40-campos-comerciais-avisos-brief");
    expect(entries[5].frontmatter.date).toBe("2026-08-14");
    expect(entries[6].frontmatter.id).toBe("vendeo-em-novo-endereco-acesso-simples");
    expect(entries[6].frontmatter.date).toBe("2026-08-12");

...
describe("get-changelog", () => {
  it("getAllEntries lê o diretório real e retorna entries ordenadas por data DESC (F42 2026-08-19 → F41 2026-08-15 → F40 2026-08-14 → novo-endereço 2026-08-12 → F36 → F35)", async () => {
    ...
    expect(entries[0].frontmatter.id).toBe("criacao-campanhas-mais-clara-confiavel");
    expect(entries[0].frontmatter.date).toBe("2026-08-20");
    expect(entries[1].frontmatter.id).toBe("fase-42-signup-controlado-elegibilidade-freemium");
    expect(entries[1].frontmatter.date).toBe("2026-08-19");
    expect(entries[2].frontmatter.id).toBe("fase-41-midia-de-campanha-mobile");
    expect(entries[2].frontmatter.date).toBe("2026-08-15");
    expect(entries[3].frontmatter.id).toBe("fase-40-campos-comerciais-avisos-brief");
    expect(entries[3].frontmatter.date).toBe("2026-08-14");
    expect(entries[4].frontmatter.id).toBe("vendeo-em-novo-endereco-acesso-simples");
    expect(entries[4].frontmatter.date).toBe("2026-08-12");
  ...
  it("getLatestAnnouncement retorna a entry mais recente com anúncio no diretório real (F42 2026-08-19)", async () => {
    ...
    expect(result).not.toBeNull();
    expect(result!.frontmatter.id).toBe("criacao-campanhas-mais-clara-confiavel");

...
const MAX_RETRIES = 1;
const BASE_URL = "https://api.cnpja.com.br/companies";
...
function mapResponse(data: Record<string, unknown>, cnpj: string): CnpjLookupData {
  ...
  const mainActivity = (data.main_activity as Record<string, unknown>) || null;
  const status = data.status as Record<string, unknown> | undefined;

  return {
    cnpj_normalized: cnpj,
    razao_social: (data.alias as string) || (company?.name as string) || "",
    nome_fantasia: (data.name as string) || null,

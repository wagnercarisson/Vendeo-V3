...
describe("F50 PostgreSQL idempotency, permissions, eligibility and wrappers", () => {
  ...
  it("deduplicates notification/event writes and enforces service-role-only RPCs", async () => {
    ...
    const dedupKey = `same-${crypto.randomUUID()}`;
    await q("insert into credit_notifications (store_id, kind, dedup_key) values ($1, 'demo_granted', $2) on conflict do nothing", [store, dedupKey]);
    await q("insert into credit_notifications (store_id, kind, dedup_key) values ($1, 'demo_granted', $2) on conflict do nothing", [store, dedupKey]);
    expect((await q("select count(*)::int as count from credit_notifications where store_id = $1", [store]))[0].count).toBe(1);
    await q("insert into product_events (store_id, user_id, event_type, dedup_key, properties) values ($1, (select user_id from stores where id = $1), 'demo_granted', $2, '{}') on conflict do nothing", [store, dedupKey]);
    await q("insert into product_events (store_id, user_id, event_type, dedup_key, properties) values ($1, (select user_id from stores where id = $1), 'demo_granted', $2, '{}') on conflict do nothing", [store, dedupKey]);
    expect((await q("select count(*)::int as count from product_events where store_id = $1 and event_type = 'demo_granted' and dedup_key = $2", [store, dedupKey]))[0].count).toBe(1);

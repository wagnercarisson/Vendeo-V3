...
BEGIN
  ...
  SELECT COALESCE(jsonb_agg(row_data ORDER BY au.created_at DESC), '[]'::jsonb) INTO v_data
  FROM (
    SELECT jsonb_build_object(
      'userId', au.id,
      'email', au.email,
      ...
      'totalCampaigns', COALESCE(c.campaign_count, 0),
      'errorCampaigns', COALESCE(ec.error_count, 0),
      'lastCampaignAt', c_last.last_campaign_at,
  ...
  LEFT JOIN (
    SELECT store_id, COUNT(*) AS campaign_count
    FROM public.campaigns
    GROUP BY store_id
  ) c ON c.store_id = s.id
  LEFT JOIN (
    SELECT store_id, COUNT(*) AS error_count
    FROM public.campaigns
    WHERE status = 'error'
    GROUP BY store_id
  ) ec ON ec.store_id = s.id
  LEFT JOIN (
    SELECT store_id, MAX(created_at) AS last_campaign_at
    FROM public.campaigns
    GROUP BY store_id
  ) c_last ON c_last.store_id = s.id
  WHERE (p_search IS NULL
    OR au.email ILIKE '%' || p_search || '%'
    OR s.name ILIKE '%' || p_search || '%'
    OR s.segment ILIKE '%' || p_search || '%')

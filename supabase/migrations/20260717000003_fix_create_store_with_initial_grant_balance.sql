-- Fix: column reference "balance" was ambiguous because WHERE store_id = store_id
-- compared the column to itself instead of column = local variable.
-- Qualify the column name to disambiguate.
-- See: 25-UAT.md issue #1 (UAT Test 1 failed with 500)
...
BEGIN
  ...
  -- FIXED: qualify column name to disambiguate from local variable

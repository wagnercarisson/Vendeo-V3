-- F38.1 FIX — regeneracoes nunca negativas e baseadas apenas em etapas de arte
-- =============================================================================
-- Correção do bug reportado em /api/admin/ai-costs: `regeneracoes` retornava -1
-- para runs de campanha.
--
-- Causa: `regeneracoes = MAX(attempt_number) - 1` calculado sobre TODOS os eventos
-- call-level. Eventos como `campaign_input_validation` gravam attempt_number 0 e
-- `campaign_copy` grava attempt_number 1 — contaminando o MAX (0 no run só de
-- validação → 0 - 1 = -1) e inflando a contagem com etapas que não são tentativa
-- de arte.
--
-- Correção aplicada nos 3 artefatos (view apuração, view reconciliação, RPC):
--   regeneracoes = GREATEST(COALESCE(MAX(attempt_number) FILTER (
--     WHERE generation_type IN (etapas de arte)
--   ), 0) - 1, 0)
--   Etapas de arte por domínio:
--     - campanha: campaign_image, campaign_image_review
--     - VS:       visual_signature_image, visual_signature_validation
--   Runs só com validação/input-check (sem etapa de arte) → regeneracoes = 0.
--   Regenerações nunca negativas (GREATEST(..., 0) — floor no mínimo 0).
-- =============================================================================

-- 1. admin_ai_operation_costs — nova coluna regeneracoes (etapas de arte, >= 0)
...
-- 2. admin_cost_vs_credits — regeneracoes por etapas de arte do domínio, >= 0
...
-- 3. admin_get_ai_costs — by_operation_run.regeneracoes por etapas de arte, >= 0
...
-- =============================================================================
-- REVERT (ordem reversa — volta à definição pré-fix da migration 20260808000001)
-- =============================================================================
...
-- CREATE OR REPLACE VIEW public.admin_cost_vs_credits AS (definição original da 20260808000001);
-- CREATE OR REPLACE VIEW public.admin_ai_operation_costs AS (definição original da 20260808000001);

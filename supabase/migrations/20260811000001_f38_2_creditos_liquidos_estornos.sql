-- F38.2 GAP — créditos estornados/líquidos por run nos RPCs de apuração
-- =============================================================================
-- Fechamento do gap UAT F38.2 (estornos não descontados): falhas debitam
-- crédito e depois estornam (refund_credit grava `reference` → deduction
-- original, `campaign_id` NULL — 20260722000002:358-365), mas os KPIs
-- contabilizavam deduções sem descontar estornos — superestimando
-- receita/resultado/margem. Correção SÓ nos RPCs da F38.2 (camada de
-- apuração), reutilizando a lógica de dedução da view e somando os refunds
-- (`ct.type = 'refund' AND ct.reference = deduction.id::text`).
--
-- Regras:
--   - NÃO toca em public.admin_cost_vs_credits (view F38.1 — contrato compat)
--     nem em admin_get_ai_costs / admin_get_metrics nem em migration antiga.
--     Fix é exclusivamente CREATE OR REPLACE dos DOIS RPCs da F38.2.
--   - `creditos_debitados` mantém semântica BRUTO (auditoria de deduções) —
--     o valor do LEFT JOIN admin_cost_vs_credits NÃO muda.
--   - `creditos_liquidos = max(creditos_debitados - creditos_estornados, 0)`
--     computado no SQL (floor em 0 — nunca negativo, T-38.2-G01).
--   - Custo de IA NÃO é estornado — falha após consumo de IA continua
--     contando como custo (apenas a receita zera).
--   - Refunds lidos direto do ledger (credit_transactions) via reference —
--     append-only por desenho (T-38.2-G04).
--   - Mesmo shape de âncora ao run da CTE `de` existente (campanha via
--     campaign_id + feature=campaign_pipeline; VS via credit_tx_id em
--     store_visual_signatures.metadata) — T-38.2-G01.
--   - Parâmetros vinculados p_* (nunca concatenação — T-38.2-G03).
--
-- Blocos:
--   1. RPC admin_get_ai_operation_runs (lista) — creditos_estornados/creditos_liquidos por run + summary
--   2. RPC admin_get_ai_operation_run_events (detalhe) — 3 campos de crédito no run
--   3. Reversão (comentada) em ordem reversa
-- =============================================================================

-- =============================================================================
-- 1. RPC admin_get_ai_operation_runs (lista por entrega) — estornos/líquidos
-- =============================================================================
...
'RPC definer — lista entregas (operation_run_id) com filtros, paginação e summary sobre o conjunto filtrado (antes da página). Expõe evidências brutas de segmento (D9), insumos de badge (D5) e créditos por run: creditos_debitados (BRUTO — view F38.1 admin_cost_vs_credits), creditos_estornados (refunds via reference → deduction no ledger) e creditos_liquidos = max(bruto − estorno, 0). Classificação é do service. Janela máx 365 dias. Acesso exclusivo service_role.';
...
-- =============================================================================
-- 2. RPC admin_get_ai_operation_run_events (detalhe call-level) — 3 campos de crédito
-- =============================================================================
...
'RPC definer — detalhe call-level de um run (operation_run_id): eventos com tokens/duração/custo/status + P95 de duração por chamada + créditos por run (creditos_debitados BRUTO, creditos_estornados via refunds no ledger, creditos_liquidos = max(bruto − estorno, 0)). run NULL + events [] quando o id não existe. Acesso exclusivo service_role.';
